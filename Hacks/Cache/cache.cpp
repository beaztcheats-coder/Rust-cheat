    ...
    while (true) {
        ...
        for (auto i = 0; i < entity_size; i++) {
            ...
            if (isPlayer) {
                ...
                if (WORLD::Hemp && hempOk && (low.find("hemp-collectable.prefab") != std::string::npos || low.find("hemp.entity.prefab") != std::string::npos))
                {
                    Rust::PrefabData data;
                    data.name = TR("Hemp");
                ...
                if (WORLD::LargeStorage && (low.find("storage_barrel_") != std::string::npos || low.find("box.wooden.large.prefab") != std::string::npos || low.find("coffinstorage.prefab") != std::string::npos || low.find("woodbox_deployed.prefab") != std::string::npos))
                ...
                if (WORLD::CargoShip && low.find("cargoship") != std::string::npos)
...
void cache::do_PositionRefresh() {
    ...
    while (true) {
        ...
        // Local player data — always read neck transform for aimbot accuracy + crouch state
        Vector3 cachedLocalNeck = {};
        Vector3 cachedBodyAngles = {};
        bool cachedIsCrouching = false;
        {
            // Always read neck position — needed for aimbot source position
            auto localNeck = src->LocalPlayer->Get_Transformation(BoneList::neck);
            if (localNeck) cachedLocalNeck = localNeck->Get_Position();

            // Read crouch state from ModelState — needed for aimbot fallback offset
            uintptr_t msPtr = read<uintptr_t>((uintptr_t)src->LocalPlayer + offsets::BasePlayer::ModelState);
            if (msPtr && is_valid(msPtr)) {
                int msFlags = read<int>(msPtr + offsets::ModelState::flags);
                cachedIsCrouching = (msFlags & offsets::ModelState::Ducked) != 0;
            }
            ...
            cachedTeam = read<uint64_t>((uintptr_t)src->LocalPlayer + offsets::BasePlayer::CurrentTeam);
        {
            ...
            g_LocalIsCrouching = cachedIsCrouching;
        }
