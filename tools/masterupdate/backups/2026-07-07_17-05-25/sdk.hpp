...
namespace decrypt {
    ...
    inline uintptr_t networkable_key(uint64_t a1)
    {
        ...
        do {
            uint32_t x = *data;
            uint32_t v1 = (x << OffsetManager::DecryptCfg.nk_rol1) | (x >> (32 - OffsetManager::DecryptCfg.nk_rol1)); // ROL
            uint32_t v2 = v1 ^ OffsetManager::DecryptCfg.nk_xor; // XOR
            uint32_t v3 = (v2 << OffsetManager::DecryptCfg.nk_rol2) | (v2 >> (32 - OffsetManager::DecryptCfg.nk_rol2)); // ROL
            *data = v3;
            data++;
            --count;
        } while (count);
    ...
    // cl_active_item: add-rol-add, count=1 (build 24087225)
    inline uint64_t decrypt_ClActiveItem(uint64_t raw_value)
    {
        ...
        do {
            uint32_t x = *data;
            uint32_t v1 = x + OffsetManager::DecryptCfg.cla_add1; // ADD
            uint32_t v2 = (v1 << OffsetManager::DecryptCfg.cla_rol) | (v1 >> (32 - OffsetManager::DecryptCfg.cla_rol)); // ROL
            uint32_t v3 = v2 + OffsetManager::DecryptCfg.cla_add2; // ADD
            *data = v3;
            data++;
            --count;
        } while (count);
    ...
    // player_inventory: add-rol-add-rol (build 24087225)
    inline uint64_t decrypt_inventory_pointer(uint64_t raw_value)
    {
        ...
        do {
            uint32_t x = *data;
            uint32_t v1 = x + OffsetManager::DecryptCfg.inv_add1; // ADD
            uint32_t v2 = (v1 << OffsetManager::DecryptCfg.inv_rol1) | (v1 >> (32 - OffsetManager::DecryptCfg.inv_rol1)); // ROL
            uint32_t v3 = v2 + OffsetManager::DecryptCfg.inv_add2; // ADD
    ...
    // player_eyes: add-xor-rol-xor (build 24087225)
    inline uint64_t decrypt_eyes(uint64_t raw_value)
    {
        ...
        do {
            ...
            uint32_t v2 = v1 ^ OffsetManager::DecryptCfg.ey_xor1; // XOR
            uint32_t v3 = (v2 << OffsetManager::DecryptCfg.ey_rol) | (v2 >> (32 - OffsetManager::DecryptCfg.ey_rol)); // ROL
            uint32_t v4 = v3 ^ OffsetManager::DecryptCfg.ey_xor2; // XOR
            *data = v4;
            data++;
            --count;
        } while (count);
    ...
    inline uint32_t decrypt_fov(uint32_t val) {
        ...
        val += OffsetManager::DecryptCfg.fov_add2;
        return val;
    }

    // encrypt_fov: reverse — sub-ror-sub (build 24087225)
