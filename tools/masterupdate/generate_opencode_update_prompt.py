...
def main():
    ...
    if morphine_offline:
        a("")
        a("!!! MORPHINE WAS OFFLINE — DATA SOURCE: SHA-DUMPER ONLY !!!")
        a("All offsets and decrypt constants came from the sha-dumper runtime dump.")
        a("These are generally accurate but lack Morphine cross-verification.")
        a("PRIORITY: Verify these decrypt constants before distribution:")
        a("  - nk_rol, nk_xor, nk_add (networkable_key)")
        a("  - nk2_rol, nk2_add, nk2_xor (networkable_key2)")
        a("  - cla_* (ClActiveItem), inv_* (inventory pointer)")
        a("Method: inject with debug logging, verify BN chain SUCCESS + held item works.")
    a("")
    a("Decrypt functions changed in this update:")
    if decrypt_changes:
        for d in decrypt_changes[:30]:
            a(f"  - {d}")
    else:
        a("  (none detected in diff_report.txt, or diff section empty)")
    a("")
    ...
    a("")
    a("Pipeline outputs (what changed this update):")
    a(f"  2. {out_dir / 'diff_report.txt'}")
    a(f"  3. {out_dir / 'summary.txt'}")
    a(f"  4. {out_dir / 'offsets_patch.txt'}")
    a(f"  5. {out_dir / 'sdk_patch.txt'}")
    a(f"  6. {out_dir / 'sdk_decrypt_patch.txt'}")
    a(f"  7. {out_dir / 'offsetmanager_patch.txt'}")
    a(f"  8. {out_dir / 'rust_decrypts.dat'}")
    a(f"  9. {out_dir / 'master.json'}  (structured offset data)")
    a(f" 10. {out_dir / 'super_prompt.txt'}  (compact per-field notes)")
    a("")
    a("Source files you will patch manually:")
    a(f" 11. {cfg['offsets_hpp']}")
    a(f" 12. {cfg['sdk_hpp']}")
    a(f" 13. {Path(cfg['offsets_hpp']).parent / 'OffsetManager.hpp'}")
    a(f" 14. {project_root / 'Hacks' / 'Cache' / 'cache.cpp'}")
    a(f" 15. {project_root / 'Hacks' / 'Misc' / 'misc.cpp'}")
    a(f" 16. {project_root / 'Hacks' / 'Aimbot' / 'aimbot.cpp'}")
    a(f" 17. {project_root / 'Hacks' / 'Visuals' / 'visuals.cpp'}")
    a(f" 18. {project_root / 'Hacks' / 'Cheat' / 'Cheat.hpp'}")
    a(f" 19. {project_root / 'Render' / 'render.hpp'}")
    a(f" 20. {project_root / 'Config.hpp'}")
    a("")
    if dump_path:
        a("Verification dump:")
        a(f" 21. {dump_path}  (verify offsets against this -- do NOT trust UnknownCheats)")
        a("")
    ...
    if unmapped_ns:
        ...
        a("  (none detected)")
    a("Specific hardcoded offsets to verify against dump.cs:")
    ...
    a("Verify each against dump.cs. Pay special attention to:")
    ...
    a("3. Check the flavor-specific log:")
    ...
    a("7. Test aimbot, recoil, and FOV changer individually.")
    if has_mesh:
    ...
    a("- Never trust UnknownCheats offsets blindly; verify against the local dump.cs.")
    ...
    a("INSTRUCTIONS")
    a("================================================================")
    a("1. Read every file listed in Section 2 (in order).")
    a("2. Compare the manual offsets in Section 3 against dump.cs.")
    a("3. Apply ONLY the minimal patches needed. Do not refactor.")
    a("4. Build after each batch of edits. Use compiler errors as your next input.")
    a("5. When the build is green, report: what you changed, what still needs testing,")
    a("   and any offsets you could not verify.")
    a("")
    ...
    print(f"     Build: {build}  |  {len(lines)} lines  |  "
          f"unmapped namespaces: {len(unmapped_ns)}  |  "
          f"ask_opencode absorbed: {'yes' if ask_content else 'no'}")
