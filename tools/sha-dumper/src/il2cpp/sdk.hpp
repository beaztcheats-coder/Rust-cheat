...
#include "util.hpp"
#include "internal/il2cpp-class-internals.h"
#include "internal/il2cpp-tabledefs.h"
...
namespace il2cpp {
	...
	inline bool is_valid_ptr(uint64_t p) {
		return p > 0x10000 && p < 0x7FFFFFFFFFFull && !IsBadReadPtr((const void*)p, 8);
	}
...
#define MAX_TYPES 16
	...
	inline field_info_t* get_field_by_name_traverse( il2cpp_class_t* klass, const char* name )
	{
		while ( klass && is_valid_ptr( klass ) )
		{
			field_info_t* field = get_field_by_name( klass, name );
			if ( field )
				return field;
			klass = klass->parent();
		}
