...
static std::uint64_t g_player_inventory_hv_offset = 0;
static std::uint64_t g_player_eyes_hv_offset = 0;
static bool g_cl_active_item_verified = false;
static bool g_player_inventory_verified = false;
static bool g_player_eyes_verified = false;
...
static std::uint64_t class_field_offset_by_name_contains_type( il2cpp::il2cpp_class_t* klass, const char* a, const char* b, const char* type );
...
static std::uint64_t first_field_offset_between_type( il2cpp::il2cpp_class_t* klass, std::uint64_t min_offset, std::uint64_t max_offset, const char* type );
static std::uint64_t nth_field_offset_between_type( il2cpp::il2cpp_class_t* klass, std::uint64_t min_offset, std::uint64_t max_offset, const char* type, std::size_t index );
static std::uint64_t last_field_offset_between_type( il2cpp::il2cpp_class_t* klass, std::uint64_t min_offset, std::uint64_t max_offset, const char* type );
...
static il2cpp::il2cpp_class_t* class_with_field_names( const char* a, const char* b = nullptr );
...
static std::uint64_t static_class_rva_exact( il2cpp::il2cpp_class_t* klass );
...
static const char* const k_player_inventory_sigs[] = {
	"40 53 57 48 83 EC 38 48 83 7A ? ? 48 8B DA 48 8B F9 75 ? 48 8B CA E8 ? ? ? ? 48 85 FF 0F 84 ? ? ? ? 80 7F ? ? 74 ? 48 8B 47 ? 48 8D 54 24 ? 48 89 44 24 ? 41 B8 02 00 00 00 90 8B 02 48 8D 52 ? 05 72 3B F8 86",
};

static const char* const k_cl_active_item_sigs[] = {
	"40 53 57 48 83 EC 38 48 83 7A ? ? 48 8B DA 48 8B F9 75 ? 48 8B CA E8 ? ? ? ? 48 85 FF 0F 84 ? ? ? ? 80 7F ? ? 74 ? 48 8B 47 ? 48 8D 54 24 ? 48 89 44 24 ? 41 B8 02 00 00 00 90 8B 02 48 8D 52 ? 05 E6 52 9D E4",
};

static const char* const k_player_eyes_sigs[] = {
	"40 53 57 48 83 EC 38 48 83 7A ? ? 48 8B DA 48 8B F9 75 ? 48 8B CA E8 ? ? ? ? 48 85 FF 0F 84 ? ? ? ? 80 7F ? ? 74 ? 48 8B 47 ? 48 8D 54 24 ? 48 89 44 24 ? 41 B8 02 00 00 00 90 8B 0A 8B 02 48 8D 52 ? C1 E9 1B",
};

static const char* const k_fov_encrypt_sigs[] = {
	"48 83 EC 58 48 C7 44 24 ? ? ? ? ? 48 85 D2 74 ? 83 3D ? ? ? ? ? 48 89 54 24 ? 74 ? 4C 8D 44 24 ? 49 C1 E8 0C 4C 8D 0D ? ? ? ? 41 81 E0 FF FF 1F 00 41 8B D0 48 C1 EA 06 41 83 E0 3F 41 0F 0D 0C D1 66 0F 1F 84 00 ? ? ? ? 49 8B 04 D1 48 8B C8 4C 0F AB C1 F0 49 0F B1 0C D1 75 ? 48 8B 54 24 ? C7 44 24 ? ? ? ? ? 48 85 D2 0F 84 ? ? ? ? 8B 42 ? 89 44 24 ? 0F 28 44 24 ? EB ? 0F 57 C0 0F 29 44 24 ? 45 33 C0",
};
...
struct base_player_live_offsets_t
{
	...
	std::uint64_t player_model_is_npc = 0;
	int player_model_is_npc_true = 0;
	int player_model_is_npc_false = 0;
	int player_model_is_npc_invalid = 0;
	int player_model_is_npc_refs = 0;
};
...
static bool instruction_references_offset( const hde64s& hs, std::uint64_t offset )
{
	if ( ( hs.flags & F_DISP32 ) && hs.disp.disp32 == ( std::uint32_t )offset )
		return true;
	return offset <= 0x7F &&
		( hs.flags & F_DISP8 ) &&
		( std::int8_t )hs.disp.disp8 == ( std::int64_t )offset;
}
...
static bool fn_references_offset( std::uintptr_t fn, std::uint64_t offset, std::size_t scan_size = 0x300 )
{
	...
	while ( p < end )
	{
		...
		if ( instruction_references_offset( hs, offset ) )
			return true;
...
static std::uint64_t hidden_value_payload_offset( std::uintptr_t fn, std::size_t scan_size = 0x100 );
...
static bool fn_has_hidden_value_decrypt_shape( std::uintptr_t fn, std::size_t scan_size = 0x300 )
{
	...
	bool has_handle_value = hidden_value_payload_offset( fn, scan_size ) != 0;
...
static std::uint64_t hidden_value_payload_offset( std::uintptr_t fn, std::size_t scan_size )
{
	if ( !fn || IsBadReadPtr( ( void* )fn, scan_size ) )
		return 0;
	...
	while ( p < end )
	{
		...
		if ( hs.rex_w && hs.opcode == 0x8B && hs.modrm_mod != 3 &&
			( ( hs.flags & F_DISP8 ) || ( hs.flags & F_DISP32 ) ) )
		{
			auto disp = ( hs.flags & F_DISP32 )
				? ( std::int64_t )( std::int32_t )hs.disp.disp32
				: ( std::int64_t )( std::int8_t )hs.disp.disp8;
			if ( disp >= 8 && disp <= 0x80 )
				payload_offset = ( std::uint64_t )disp;
		}

		auto starts_two_half_loop =
			hs.opcode >= 0xB8 && hs.opcode <= 0xBF &&
			( hs.flags & F_IMM32 ) && hs.imm.imm32 == 2;
		if ( p + 6 <= end && p[ 0 ] == 0x41 && p[ 1 ] >= 0xB8 && p[ 1 ] <= 0xBF &&
			p[ 2 ] == 0x02 && p[ 3 ] == 0x00 && p[ 4 ] == 0x00 && p[ 5 ] == 0x00 )
			starts_two_half_loop = true;
		if ( starts_two_half_loop )
...
static void print_decryption_ops(
	const char* name,
	std::uintptr_t fn_addr,
	const c_decryption::constants_t& constants,
	std::uint64_t payload_offset = 0 )
{
	std::printf(
		"[morphine-dumper] decrypt %s: rva=0x%llX payload=0x%llX ops=%d unrolled=%d\n",
		name,
		fn_addr ? fn_addr - c_runtime::game_base( ) : 0,
		payload_offset,
		constants.op_count,
		constants.was_unrolled ? 1 : 0 );
	for ( int i = 0; i < constants.op_count; i++ )
		std::printf(
			"[morphine-dumper] decrypt %s: op[%d]=%s 0x%X\n",
			name,
			i,
			c_decryption::op_name( constants.ops[ i ].type ),
			constants.ops[ i ].value );
}

static void print_identity_u32_decryption( const char* name )
{
	std::printf(
		"[morphine-dumper] decrypt %s selected: rva=0x0 payload=0x0 ops=0 identity=low32\n",
		name );
}
...
{
	if ( !base_networkable_rva || !field_hv_offset || !decrypt_0.valid || !decrypt_1.valid ||
	...
	for ( std::int32_t i = 0; i < limit; i++ )
	{
		...
		auto resolved = decrypt_handle( entity + field_offset, field_hv_offset, field_decrypt );
		...
		{
			resolved = decrypt_handle( direct, field_hv_offset, field_decrypt );
			if ( object_is_class( resolved, target_class ) )
				hits++;
		}
	}
	...
	std::uint32_t* out_container_hits )
{
	if ( !base_networkable_rva || !decrypt_0.valid || !decrypt_1.valid ||
		!inventory_hv_offset || !inventory_decrypt.valid ||
	...
	auto item_has_uid = [&]( std::uintptr_t item, std::uint64_t uid ) -> bool
	{
		...
		auto value_matches_uid = [&]( std::uint64_t stored ) -> bool
		{
			if ( uid_matches( stored, uid ) )
				return true;
			return active_item_decrypt.valid &&
				uid_matches( c_decryption::simulate( stored, active_item_decrypt ), uid );
		};
	...
	auto push_object_uids = [&]( std::uintptr_t object, auto&& push_uid )
	{
		if ( !is_valid_userland_ptr( object ) )
			return;
		...
		for ( auto off : preferred_offsets )
		{
			if ( !off || !is_valid_userland_ptr( object + off ) )
				continue;
			push_uid( *( std::uint32_t* )( object + off ) );
			if ( is_valid_userland_ptr( object + off + sizeof( std::uint64_t ) - 1 ) )
				push_uid( *( std::uint64_t* )( object + off ) );
		}

		for ( std::uint32_t off = 0x10; off <= 0x160; off += 4 )
		{
			if ( !is_valid_userland_ptr( object + off ) )
				continue;

			push_uid( *( std::uint32_t* )( object + off ) );
			if ( ( off & 7 ) == 0 &&
				is_valid_userland_ptr( object + off + sizeof( std::uint64_t ) - 1 ) )
				push_uid( *( std::uint64_t* )( object + off ) );
		}
	};
	...
	for ( std::int32_t i = 0; i < limit; i++ )
	{
		...
		if ( !is_valid_userland_ptr( entity + cl_active_item_offset + sizeof( std::uint64_t ) - 1 ) )
			continue;

		auto active_raw = *( std::uint64_t* )( entity + cl_active_item_offset );
		std::uint64_t active_uids[ 128 ]{ };
		std::uint32_t active_uid_count = 0;
		auto push_active_uid = [&]( std::uint64_t uid )
		{
			if ( !uid || uid == ~0ull || active_uid_count >= 128 )
				return;
			for ( std::uint32_t j = 0; j < active_uid_count; j++ )
				if ( active_uids[ j ] == uid )
					return;
			active_uids[ active_uid_count++ ] = uid;
		};

		auto active_low = ( std::uint32_t )active_raw;
		auto active_high = ( std::uint32_t )( active_raw >> 32 );
		if ( active_item_decrypt.valid )
		{
			push_active_uid( apply_u32_ops( active_low, active_item_decrypt, false ) );
			push_active_uid( apply_u32_ops( active_high, active_item_decrypt, false ) );
		}
		else
		{
			push_active_uid( active_low );
			push_active_uid( active_high );
			push_active_uid( active_raw );
		}

		for ( std::uint64_t active_off = cl_active_item_offset;
			active_off <= cl_active_item_offset + 0x20;
			active_off += 4 )
		{
			if ( !is_valid_userland_ptr( entity + active_off ) )
				continue;

			auto dword = *( std::uint32_t* )( entity + active_off );
			push_active_uid( dword );
			if ( active_item_decrypt.valid )
				push_active_uid( apply_u32_ops( dword, active_item_decrypt, false ) );

			if ( ( active_off & 7 ) == 0 &&
				is_valid_userland_ptr( entity + active_off + sizeof( std::uint64_t ) - 1 ) )
			{
				auto qword = *( std::uint64_t* )( entity + active_off );
				push_active_uid( qword );
				push_active_uid( ( std::uint32_t )qword );
				push_active_uid( ( std::uint32_t )( qword >> 32 ) );
				if ( active_item_decrypt.valid )
				{
					push_active_uid( apply_u32_ops( ( std::uint32_t )qword, active_item_decrypt, false ) );
					push_active_uid( apply_u32_ops( ( std::uint32_t )( qword >> 32 ), active_item_decrypt, false ) );
				}
			}
		}
		...
		if ( active_item_decrypt.valid && active_item_hv_offset )
		{
			auto active_wrapper = read_ptr_safe( entity + cl_active_item_offset );
			push_object_uids( active_wrapper, push_active_uid );

			auto active_object = decrypt_handle( active_wrapper, active_item_hv_offset, active_item_decrypt );
			push_object_uids( active_object, push_active_uid );

			auto inline_active_object = decrypt_handle( entity + cl_active_item_offset, active_item_hv_offset, active_item_decrypt );
			push_object_uids( inline_active_object, push_active_uid );
		}
		...
		{
			inventory = decrypt_handle( entity + player_inventory_off, inventory_hv_offset, inventory_decrypt );
			if ( !object_is_class( inventory, player_inventory_class ) )
			{
				auto inv_wrapper = *( std::uintptr_t* )( entity + player_inventory_off );
				if ( is_valid_userland_ptr( inv_wrapper ) )
					inventory = decrypt_handle( inv_wrapper, inventory_hv_offset, inventory_decrypt );
			}
		}
		if ( !object_is_class( inventory, player_inventory_class ) )
			continue;
		...
		for ( auto container_off : containers )
		{
			...
			for ( std::uint32_t active_uid_index = 0; active_uid_index < active_uid_count; active_uid_index++ )
			{
				if ( list_contains_uid( list, active_uids[ active_uid_index ] ) )
				{
					hits++;
					matched = true;
					break;
...
static bool managed_string_to_ascii( std::uintptr_t str, std::string& out )
{
	out.clear( );
	if ( !is_valid_userland_ptr( str ) )
		return false;
	...
	__try
	{
		...
		{
			auto ch = chars[ i ];
			out.push_back( ch >= 0x20 && ch <= 0x7E ? ( char )ch : '_' );
		}
		return true;
	}
	__except ( EXCEPTION_EXECUTE_HANDLER )
...
void c_core::resolve_model_bones( )
{
	...
	for ( std::int32_t i = 0; i < limit; i++ )
	{
		...
		if ( bone_count > 0 && bone_count < 256 )
		{
			...
			if ( m_model_bone_names.empty( ) )
			{
				auto data = bone_names + 0x20;
				for ( std::int32_t j = 0; j < bone_count; j++ )
				{
					auto str = *( std::uintptr_t* )( data + ( j * 8 ) );
					std::string bone_name;
					if ( managed_string_to_ascii( str, bone_name ) && !bone_name.empty( ) )
						m_model_bone_names.push_back( bone_name );
					else
						m_model_bone_names.push_back( "bone" );
				}
			}
		}
...
static std::uint64_t resolve_player_model_is_npc_offset(
	il2cpp::il2cpp_class_t* klass,
	std::uint64_t skinned_multi_mesh,
	int* refs_out = nullptr )
{
	if ( refs_out )
		*refs_out = 0;
	if ( !klass )
	...
	if ( !is_npc )
		is_npc = method_field_offset_by_name_contains( klass, "NPC" );
	if ( is_npc && skinned_multi_mesh && is_npc < skinned_multi_mesh )
		is_npc = 0;
	if ( is_npc )
		return is_npc;
	...
	std::vector<bool_ref_t> bool_refs;
	for ( auto cur = klass; cur; cur = cur->parent( ) )
	{
		...
		while ( auto f = cur->fields( &field_iter ) )
		{
			...
			if ( !tn || !ascii_contains_i( tn, "Boolean" ) )
				continue;
			auto off = f->offset( cur );
			bool_refs.push_back( { off, 0, skinned_multi_mesh && off > skinned_multi_mesh } );
		}
	}
	...
	while ( auto method = klass->methods( &method_iter ) )
	{
		...
		if ( !fn )
			continue;
		for ( auto& candidate : bool_refs )
			if ( fn_references_offset( fn, candidate.off ) )
				candidate.refs++;
	}

	bool_ref_t best{ 0, 0, false };
	for ( const auto& candidate : bool_refs )
	{
		if ( !best.off ||
			( candidate.after_mesh && !best.after_mesh ) ||
			( candidate.after_mesh == best.after_mesh && candidate.refs > best.refs ) ||
			( candidate.after_mesh == best.after_mesh && candidate.refs == best.refs && candidate.off > best.off ) )
			best = candidate;
	}
...
struct player_model_motion_fields_t
{
	std::uint64_t position = 0;
	std::uint64_t new_velocity = 0;
};

static player_model_motion_fields_t resolve_player_model_motion_fields(
	il2cpp::il2cpp_class_t* klass,
	std::uint64_t full_mask,
	std::uint64_t skinned_multi_mesh )
{
	player_model_motion_fields_t result{ };
	if ( !klass )
		return result;

	auto collect_offsets = [&]( bool bounded )
	{
		std::vector<std::uint64_t> offsets;
		if ( bounded && ( !full_mask || !skinned_multi_mesh || full_mask >= skinned_multi_mesh ) )
			return offsets;
		...
		for ( auto cur = klass; cur; cur = cur->parent( ) )
		{
			...
			while ( auto field = cur->fields( &iter ) )
			{
				if ( field->flags( ) & FIELD_ATTRIBUTE_STATIC )
					continue;
				auto off = field->offset( cur );
				if ( bounded && ( off <= full_mask || off >= skinned_multi_mesh ) )
					continue;
				if ( field_type_matches( field, "Vector3" ) )
	...
	auto resolve_from_offsets = []( const std::vector<std::uint64_t>& offsets )
	{
		player_model_motion_fields_t resolved{ };
		...
		{
			if ( run_count && off == run_end + sizeof( vec3_t ) )
			{
				run_end = off;
				run_count++;
			}
			...
			if ( run_count > best_count || ( run_count == best_count && run_end > best_end ) )
			{
		...
		if ( best_count >= 3 )
		{
			resolved.position = best_start;
			resolved.new_velocity = best_end;
			return resolved;
		}

		for ( auto i = offsets.size( ); i > 1; --i )
		{
			if ( offsets[ i - 2 ] + sizeof( vec3_t ) != offsets[ i - 1 ] )
				continue;
			resolved.position = offsets[ i - 2 ];
			resolved.new_velocity = offsets[ i - 1 ];
			return resolved;
		}

		if ( offsets.size( ) >= 2 )
		{
			resolved.position = offsets[ offsets.size( ) - 2 ];
			resolved.new_velocity = offsets.back( );
		}
		return resolved;
	};

	result = resolve_from_offsets( collect_offsets( true ) );
	if ( result.position && result.new_velocity )
		return result;

	result = resolve_from_offsets( collect_offsets( false ) );
	return result;
}
	...
	il2cpp::il2cpp_class_t* item_container_class )
{
	...
	auto player_model_full_mask = field_offset_by_name( player_model_klass, "fullMask" );
	if ( !player_model_full_mask )
		player_model_full_mask = field_offset_by_name_contains( player_model_klass, "full", "mask" );
	auto player_model_motion = resolve_player_model_motion_fields(
		player_model_klass,
		player_model_full_mask,
		player_model_skinned_multi_mesh );
	int is_npc_refs = 0;
	auto player_model_is_npc_off = resolve_player_model_is_npc_offset(
		player_model_klass,
		player_model_skinned_multi_mesh,
		&is_npc_refs );
	int is_npc_true = 0;
	int is_npc_false = 0;
	int is_npc_invalid = 0;
	...
	for ( std::int32_t i = 0; i < limit; i++ )
	{
		...
		if ( player_model_is_npc_off && is_valid_userland_ptr( player_model + player_model_is_npc_off ) )
		{
			auto value = *( std::uint8_t* )( player_model + player_model_is_npc_off );
			if ( value == 0 )
				is_npc_false++;
			else if ( value == 1 )
				is_npc_true++;
		...
		if ( player_model_motion.position && read_player_model_root_position( player_model, model_root_pos ) )
		{
			auto off = player_model_motion.position;
			if ( is_valid_userland_ptr( player_model + off + sizeof( vec3_t ) ) )
			{
				...
				if ( finite_float( value.x ) && finite_float( value.y ) && finite_float( value.z ) &&
					near_vec3( value, model_root_pos ) )
	...
	if ( player_model_is_npc_off && ( is_npc_true + is_npc_false ) >= live_min_hits )
	{
		g_base_player_live.player_model_is_npc = player_model_is_npc_off;
		g_base_player_live.player_model_is_npc_true = is_npc_true;
		g_base_player_live.player_model_is_npc_false = is_npc_false;
		g_base_player_live.player_model_is_npc_invalid = is_npc_invalid;
		g_base_player_live.player_model_is_npc_refs = is_npc_refs;
	}
	...
	std::printf(
		...
		team.hits,
		(int)team.aux );
	if ( player_model_is_npc_off )
		std::printf(
			"[morphine-dumper] player_model isNpc: offset=0x%llX true=%d false=%d invalid=%d refs=%d%s\n",
			player_model_is_npc_off,
			is_npc_true,
			is_npc_false,
			is_npc_invalid,
			is_npc_refs,
			g_base_player_live.player_model_is_npc ? "" : " unresolved" );
}

static void validate_chams_runtime(
	std::uint64_t base_networkable_rva,
	std::uint32_t bn_static_fields,
	std::uint32_t wrapper_class_ptr,
	std::uint32_t parent_static_fields,
	...
	const c_decryption::constants_t& decrypt_1 )
{
	...
	auto base_player = c_runtime::find_class( "BasePlayer" );
	auto player_model = c_runtime::find_class( "PlayerModel" );
	if ( !player_model )
		player_model = class_name_contains( "PlayerModel" );
	auto skinned_multi_mesh = find_named_class( "SkinnedMultiMesh" );
	if ( !base_player || !player_model || !skinned_multi_mesh )
	{
		std::printf( "[morphine-dumper] chams validate: unresolved classes\n" );
		return;
	}

	auto player_model_off = g_base_player_live.player_model;
	if ( !player_model_off )
		player_model_off = field_offset_by_type_class( base_player, player_model );
	if ( !player_model_off )
		player_model_off = field_offset_by_type_contains( base_player, "PlayerModel" );

	auto multi_mesh_off = field_offset_by_name( player_model, "_multiMesh" );
	if ( !multi_mesh_off )
		multi_mesh_off = field_offset_by_type_class( player_model, skinned_multi_mesh );
	if ( !multi_mesh_off )
		multi_mesh_off = field_offset_by_type_contains( player_model, "SkinnedMultiMesh" );

	auto renderer_list_off = field_offset_by_name( skinned_multi_mesh, "Renderers" );
	if ( !renderer_list_off )
		renderer_list_off = field_offset_by_name( skinned_multi_mesh, "RendererList" );
	if ( !renderer_list_off )
		renderer_list_off = field_offset_by_name_contains( skinned_multi_mesh, "renderer" );
	if ( !renderer_list_off )
		renderer_list_off = field_offset_by_type_contains( skinned_multi_mesh, "Renderer" );

	if ( !player_model_off || !multi_mesh_off || !renderer_list_off )
	{
		std::printf(
			"[morphine-dumper] chams validate: unresolved playerModel=0x%llX multiMesh=0x%llX rendererList=0x%llX\n",
			player_model_off,
			multi_mesh_off,
			renderer_list_off );
		return;
	...
	int players = 0;
	int multi_mesh_hits = 0;
	int renderer_list_hits = 0;
	int renderer_array_hits = 0;
	int first_renderer_hits = 0;
	int max_renderers = 0;
	std::uintptr_t first_renderer = 0;
	auto limit = count > 2048 ? 2048 : count;
	for ( std::int32_t i = 0; i < limit; i++ )
	{
		auto entry = *( std::uintptr_t* )( array + 0x20 + ( i * 8 ) );
		...
		if ( !is_valid_userland_ptr( entity + player_model_off ) )
			continue;
		auto model = *( std::uintptr_t* )( entity + player_model_off );
		if ( !is_valid_userland_ptr( model + multi_mesh_off ) )
			continue;

		players++;
		auto multi_mesh = *( std::uintptr_t* )( model + multi_mesh_off );
		if ( !is_valid_userland_ptr( multi_mesh + renderer_list_off ) )
			continue;
		multi_mesh_hits++;

		auto renderer_holder = *( std::uintptr_t* )( multi_mesh + renderer_list_off );
		if ( !is_valid_userland_ptr( renderer_holder + 0x20 ) )
			continue;
		renderer_list_hits++;

		std::uintptr_t renderer_array = read_ptr_safe( renderer_holder + 0x10 );
		auto renderer_count = is_valid_userland_ptr( renderer_holder + 0x18 )
			? *( std::int32_t* )( renderer_holder + 0x18 )
			: 0;
		if ( renderer_count < 0 || renderer_count > 1024 )
			continue;
		if ( renderer_count > max_renderers )
			max_renderers = renderer_count;
		if ( !renderer_count )
			continue;

		std::uintptr_t renderer = 0;
		if ( is_valid_userland_ptr( renderer_array + 0x20 ) )
		{
			renderer_array_hits++;
			renderer = *( std::uintptr_t* )( renderer_array + 0x20 );
		}
		else
		{
			renderer = *( std::uintptr_t* )( renderer_holder + 0x20 );
		}

		if ( is_valid_userland_ptr( renderer ) )
		{
			first_renderer_hits++;
			if ( !first_renderer )
	...
	std::printf(
		"[morphine-dumper] chams validate: players=%d multiMesh=0x%llX hits=%d rendererList=0x%llX listHits=%d arrayHits=%d firstRendererHits=%d maxRenderers=%d firstRenderer=0x%llX%s\n",
		players,
		multi_mesh_off,
		multi_mesh_hits,
		renderer_list_off,
		...
		first_renderer_hits,
		max_renderers,
		( unsigned long long )first_renderer,
		first_renderer_hits ? "" : " unresolved" );
}

void c_core::resolve_cl_active_item_decryption( )
{
	m_cl_active_item_fn_addr = 0;
	m_cl_active_item_offset = 0;
	m_cl_active_item_constants = { };
	m_cl_active_item_plain = false;
	g_cl_active_item_verified = false;
	...
	auto cl_active_item_field = m_item_id_class
		...
		: nullptr;
	if ( !cl_active_item_field && m_item_id_class )
	{
		auto fields = fields_by_type_class( klass, m_item_id_class );
		if ( fields.size( ) == 1 )
			cl_active_item_field = fields.front( );
	}
	auto cl_active_item_offset = cl_active_item_field ? cl_active_item_field->offset( klass ) : 0;
	auto cl_active_item_source = cl_active_item_field ? "metadata/type" : "unresolved";
	...
	std::printf( "[morphine-dumper] cl_active_item field: offset=0x%llX source=%s\n",
		cl_active_item_offset,
		cl_active_item_source );
	...
	if ( cl_active_item_offset )
	{
		...
		std::uint32_t inventory_hits = 0;
		std::uint32_t container_hits = 0;
		c_decryption::constants_t identity{ };
		auto direct_hits = score_cl_active_item_live(
		...
		if ( direct_hits )
		{
			m_cl_active_item_offset = cl_active_item_offset;
			m_cl_active_item_plain = true;
			g_cl_active_item_verified = true;
			std::printf(
				"[morphine-dumper] cl_active_item: plain UID verified field=0x%llX hits=%u active=%u inventories=%u containers=%u (falling back to candidate search)\n",
				cl_active_item_offset,
				direct_hits,
				active_values,
				inventory_hits,
	...
	auto add_candidate = [&]( std::uintptr_t addr, int score, bool field_anchored )
	{
		...
		if ( !hidden_value_shape && score < 120 )
			return;
		auto active_item_hv_offset = hidden_value_payload_offset( addr );
		...
		if ( nested_call_candidate && loop_count && loop_count != 1 && loop_count != 2 )
			return;
		...
		for ( auto test_offset : cl_active_item_offsets )
		{
			...
			if ( test_offset == cl_active_item_offset )
				candidate_score += 20;
			if ( loop_count == 1 )
				candidate_score += 2500;
			else if ( loop_count == 2 )
				candidate_score -= 500;
			if ( nested_call_candidate && ( loop_count == 1 || loop_count == 2 ) )
				candidate_score += 750;
			...
			for ( auto& existing : candidates )
			{
					...
					c_decryption::ops_match( existing.constants, constants ) )
				{
					...
					if ( active_item_hv_offset > existing.payload_offset )
						existing.payload_offset = active_item_hv_offset;
	...
	for ( const char* sig : k_cl_active_item_sigs )
		if ( auto hit = c_scanner::scan_image( sig ) )
			add_candidate( hit, 2500, false );

	if ( m_item_id_class )
	{
		void* item_id_iter = nullptr;
		while ( auto method = m_item_id_class->methods( &item_id_iter ) )
		{
			auto fn = method->get_fn_ptr<std::uintptr_t>( );
			add_candidate( fn, 700, false );
			if ( !fn || IsBadReadPtr( ( void* )fn, 0x100 ) )
				continue;
			...
			while ( p < end )
			{
				...
				if ( p[ 0 ] == 0xE8 && len >= 5 )
				{
					...
					std::memcpy( &displacement, p + 1, sizeof( displacement ) );
					add_candidate( ( std::uintptr_t )( p + 5 + displacement ), 1000, false );
				}
				p += len;
			}
		}
	...
	for ( auto method : methods )
	{
		...
		if ( m_item_id_class && ret_class == m_item_id_class )
			base_score += 40;
		auto method_anchored = fn_references_offset( fn, cl_active_item_offset );
		if ( method_anchored )
			base_score += 120;
		add_candidate( fn, base_score, method_anchored );
		...
		while ( p < end )
		{
			...
			if ( cl_active_item_offset && instruction_references_offset( hs, cl_active_item_offset ) )
			...
			if ( p[ 0 ] == 0xE8 && len >= 5 )
			{
				std::int32_t d;
				std::memcpy( &d, p + 1, 4 );
				auto anchored_call = target_offset_window > 0;
				add_candidate(
					( std::uintptr_t )( p + 5 + d ),
					base_score + ( anchored_call ? 1000 : 0 ),
					anchored_call );
			}
	...
	auto candidate_rank = []( const candidate_t& candidate ) -> int
	{
		...
		if ( candidate.active_values && candidate.inventory_hits && candidate.container_hits &&
			!candidate.hidden_value_shape && candidate.nested_call_candidate && candidate.field_anchored &&
			candidate.loop_count == 1 && candidate.score >= 1000 )
			return 2;
		return 0;
	};
	...
	for ( std::size_t i = 0; i < candidates.size( ); i++ )
	{
		...
		std::printf(
			"[morphine-dumper] cl_active_item candidate %zu: rva=0x%llX field=0x%llX payload=0x%llX rank=%d score=%d live=%u active=%u inventories=%u containers=%u hidden=%d nested=%d anchored=%d loop=%u\n",
			i,
			candidate.fn_addr - c_runtime::game_base( ),
			candidate.field_offset,
			candidate.payload_offset,
			candidate_rank( candidate ),
			candidate.score,
			candidate.live_hits,
			candidate.active_values,
			candidate.inventory_hits,
			candidate.container_hits,
			candidate.hidden_value_shape ? 1 : 0,
			candidate.nested_call_candidate ? 1 : 0,
			candidate.field_anchored ? 1 : 0,
			candidate.loop_count );
		print_decryption_ops( "clActiveItem candidate", candidate.fn_addr, candidate.constants, candidate.payload_offset );
	}
	...
	auto best_rank_value = candidate_rank( *best );
	if ( best_rank_value < 3 )
	{
		std::printf( "[morphine-dumper] cl_active_item: no UID-verified candidate found (%zu candidates)\n",
			candidates.size( ) );
		return;
	}
	...
	std::printf( "[morphine-dumper] cl_active_item: live_hits=%u score=%d\n",
		best->live_hits,
		best->score );
	print_decryption_ops( "clActiveItem selected", m_cl_active_item_fn_addr, m_cl_active_item_constants );
	if ( !best->live_hits )
...
void c_core::resolve_player_inventory_decryption( )
{
	...
	{
		auto fields = fields_by_type_class( base_player, inventory_class );
		if ( fields.size( ) == 1 )
			player_inventory_field = fields.front( );
	}
	auto player_inventory_offset = player_inventory_field ? player_inventory_field->offset( base_player ) : 0;
	std::printf( "[morphine-dumper] player_inventory field: offset=0x%llX source=%s\n",
		player_inventory_offset,
		player_inventory_field ? "metadata/type" : "unresolved" );
	...
	if ( player_inventory_offset )
	{
		...
		while ( auto method = base_player->methods( &iter ) )
		{
			...
			while ( p < end )
			{
				...
				if ( instruction_references_offset( hs, player_inventory_offset ) )
				{
					push_method( method );
					break;
				}
	...
	auto add_candidate = [&]( std::uintptr_t addr, int score, bool signature_match, bool field_anchored )
	{
		addr = resolve_jump_thunk( addr );
		if ( !addr || IsBadReadPtr( ( void* )addr, 0x100 ) )
			return;
		if ( !fn_has_hidden_value_decrypt_shape( addr ) )
			return;
		auto payload_offset = hidden_value_payload_offset( addr );
		if ( !payload_offset )
			return;
		...
		for ( auto& existing : candidates )
		{
			if ( existing.fn_addr == addr || c_decryption::ops_match( existing.constants, constants ) )
			{
				if ( live_hits > existing.live_hits || ( live_hits == existing.live_hits && score > existing.score ) )
				{
					...
					existing.live_hits = live_hits;
					existing.payload_offset = payload_offset;
				}
				existing.signature_match = existing.signature_match || signature_match;
				existing.field_anchored = existing.field_anchored || field_anchored;
				return;
			}
		}

		candidates.push_back( { addr, payload_offset, constants, score, live_hits, signature_match, field_anchored } );
	};

	for ( const char* sig : k_player_inventory_sigs )
		if ( auto hit = c_scanner::scan_image( sig ) )
			add_candidate( hit, 2500, true, false );
	...
	for ( auto method : methods )
	{
		...
		auto method_anchored = fn_references_offset( fn, player_inventory_offset );
		if ( method_anchored )
			base_score += 500;
		add_candidate( fn, base_score, false, method_anchored );
		...
		while ( p < end )
		{
			...
			if ( player_inventory_offset && instruction_references_offset( hs, player_inventory_offset ) )
			...
			if ( p[ 0 ] == 0xE8 && len >= 5 )
			{
				std::int32_t d;
				std::memcpy( &d, p + 1, 4 );
				auto field_anchored = target_offset_window > 0;
				add_candidate(
					( std::uintptr_t )( p + 5 + d ),
					base_score + ( field_anchored ? 1000 : 0 ),
					false,
					field_anchored );
	...
	if ( player_inventory_field )
	{
		...
		if ( field_klass )
		{
			...
			{
				auto fn = method->get_fn_ptr<std::uintptr_t>( );
				add_candidate( fn, 700, false, false );
				...
				while ( p < end )
				{
					...
					if ( p[ 0 ] == 0xE8 && len >= 5 )
					{
						std::int32_t d;
						std::memcpy( &d, p + 1, 4 );
						add_candidate( ( std::uintptr_t )( p + 5 + d ), 650, false, false );
					}
					p += len;
				}
			}
	...
	{
		std::printf(
			"[morphine-dumper] player_inventory candidate %zu: rva=0x%llX payload=0x%llX ops=%d score=%d live_hits=%d signature=%d anchored=%d\n",
			i,
			candidates[ i ].fn_addr - c_runtime::game_base( ),
			candidates[ i ].payload_offset,
			candidates[ i ].constants.op_count,
			candidates[ i ].score,
			candidates[ i ].live_hits,
			candidates[ i ].signature_match ? 1 : 0,
			candidates[ i ].field_anchored ? 1 : 0 );
		print_decryption_ops(
			"PlayerInventory candidate",
			candidates[ i ].fn_addr,
			candidates[ i ].constants,
			candidates[ i ].payload_offset );
	}
	for ( auto& candidate : candidates )
	{
	...
	if ( !best->live_hits )
	{
		std::printf( "[morphine-dumper] player_inventory decrypt: rejected unverified candidate\n" );
		return;
	}

	m_player_inventory_fn_addr = best->fn_addr;
	g_player_inventory_hv_offset = best->payload_offset;
	m_player_inventory_constants = best->constants;
	g_player_inventory_verified = true;
...
void c_core::resolve_player_eyes_decryption( )
{
	...
	if ( !eyes_field )
	{
		auto fields = fields_by_type_class( base_player, eyes_class );
		if ( fields.size( ) == 1 )
			eyes_field = fields.front( );
	}
	auto eyes_offset = eyes_field ? eyes_field->offset( base_player ) : 0;
	std::printf( "[morphine-dumper] player_eyes field: offset=0x%llX source=%s\n",
		eyes_offset,
		eyes_field ? "metadata/type" : "unresolved" );
	...
	if ( eyes_offset )
	{
		...
		while ( auto method = base_player->methods( &iter ) )
		{
			...
			while ( p < end )
			{
				...
				if ( instruction_references_offset( hs, eyes_offset ) )
				{
					push_method( method );
					break;
				}
	...
	for ( const char* sig : k_player_eyes_sigs )
		if ( auto hit = c_scanner::scan_image( sig ) )
			add_candidate( hit, 2500, true, false );
	...
	for ( auto method : methods )
	{
		...
		auto method_anchored = fn_references_offset( fn, eyes_offset );
		if ( method_anchored )
			base_score += 500;
		add_candidate( fn, base_score, false, method_anchored );
		...
		while ( p < end )
		{
			...
			if ( eyes_offset && instruction_references_offset( hs, eyes_offset ) )
				target_offset_window = 6;
	...
	{
		std::printf(
			"[morphine-dumper] player_eyes candidate %zu: rva=0x%llX payload=0x%llX ops=%d score=%d live_hits=%d signature=%d anchored=%d\n",
			i,
			candidates[ i ].fn_addr - c_runtime::game_base( ),
			candidates[ i ].payload_offset,
			candidates[ i ].constants.op_count,
	...
	if ( !best->live_hits )
	{
		std::printf( "[morphine-dumper] player_eyes decrypt: rejected unverified candidate\n" );
		return;
	}

	m_player_eyes_fn_addr = best->fn_addr;
	g_player_eyes_hv_offset = best->payload_offset;
	m_player_eyes_constants = best->constants;
	g_player_eyes_verified = true;
	...
	print_decryption_ops(
		"PlayerEyes selected",
		m_player_eyes_fn_addr,
		m_player_eyes_constants,
		g_player_eyes_hv_offset );
}
...
void c_core::resolve_fov_encryption( )
{
	...
	for ( const char* sig : k_fov_encrypt_sigs )
		if ( auto hit = c_scanner::scan_image( sig ) )
			add_candidate( hit, true, "ida fov encrypt signature" );
	if ( !rust::console_system::console_system_index_client_find )
	{
...
static void write_il2cpp_api( c_writer& w, std::uint64_t gc_handles_rva )
{
	...
	w.struct_begin( "GameAssembly" );
	w.struct_field( "timestamp", timestamp( ) );
	w.struct_field( "gc_handles", gc_handles_rva );
	w.struct_field( "type_info_definition_table", c_scanner::resolve_type_info_definition_table( ) );
	w.struct_field( "il2cpp_resolve_icall", export_rva( "il2cpp_resolve_icall" ) );
	w.struct_field( "il2cpp_array_new", export_rva( "il2cpp_array_new" ) );
	w.struct_field( "il2cpp_assembly_get_image", export_rva( "il2cpp_assembly_get_image" ) );
	w.struct_field( "il2cpp_class_from_name", export_rva( "il2cpp_class_from_name" ) );
	w.struct_field( "il2cpp_class_get_method_from_name", export_rva( "il2cpp_class_get_method_from_name" ) );
	w.struct_field( "il2cpp_class_get_type", export_rva( "il2cpp_class_get_type" ) );
	w.struct_field( "il2cpp_domain_get", export_rva( "il2cpp_domain_get" ) );
	w.struct_field( "il2cpp_domain_get_assemblies", export_rva( "il2cpp_domain_get_assemblies" ) );
	w.struct_field( "il2cpp_gchandle_get_target", export_rva( "il2cpp_gchandle_get_target" ) );
	w.struct_field( "il2cpp_gchandle_new", export_rva( "il2cpp_gchandle_new" ) );
	w.struct_field( "il2cpp_gchandle_free", export_rva( "il2cpp_gchandle_free" ) );
	w.struct_field( "il2cpp_method_get_name", export_rva( "il2cpp_method_get_name" ) );
	w.struct_field( "il2cpp_object_new", export_rva( "il2cpp_object_new" ) );
	w.struct_field( "il2cpp_type_get_object", export_rva( "il2cpp_type_get_object" ) );
	w.struct_end( );
	...
	w.struct_field( "class_get_method_from_name",   export_rva( "il2cpp_class_get_method_from_name" ) );
...
static il2cpp::il2cpp_class_t* find_game_manager_class( )
{
	auto domain = il2cpp::il2cpp_domain_t::get( );
	size_t assembly_count = 0;
	auto assemblies = domain ? domain->get_assemblies( &assembly_count ) : nullptr;
	for ( size_t i = 0; assemblies && i < assembly_count; i++ )
	{
		auto image = assemblies[ i ] ? assemblies[ i ]->image( ) : nullptr;
		if ( !image )
			continue;
		...
		{
			auto candidate = image->get_class( j );
			auto name = candidate ? candidate->name( ) : nullptr;
			auto parent = candidate ? candidate->parent( ) : nullptr;
			auto parent_name = parent ? parent->name( ) : nullptr;
			auto parent_type = parent ? parent->type( ) : nullptr;
			auto parent_type_name = parent_type ? parent_type->name( ) : nullptr;
			if ( name && !std::strcmp( name, "Client" ) &&
				( ascii_contains_i( parent_name, "SingletonComponent" ) ||
					ascii_contains_i( parent_type_name, "SingletonComponent" ) ) )
				return candidate;
		}
	}
	return nullptr;
...
static std::uint64_t type_definition_slot_rva(
	il2cpp::il2cpp_class_t* klass,
	std::uint64_t table_rva )
{
	if ( !klass || !table_rva )
		return 0;
	auto table_slot = c_runtime::game_base( ) + table_rva;
	if ( IsBadReadPtr( ( void* )table_slot, sizeof( std::uintptr_t ) ) )
		return 0;
	auto table = *( std::uintptr_t* )table_slot;
	if ( !is_valid_userland_ptr( table ) )
		return 0;
	...
	auto assemblies = domain ? domain->get_assemblies( &assembly_count ) : nullptr;
	for ( std::size_t i = 0; assemblies && i < assembly_count; i++ )
	{
		auto image = assemblies[ i ] ? assemblies[ i ]->image( ) : nullptr;
		if ( image )
			class_count += image->class_count( );
	}
	if ( !class_count || IsBadReadPtr( ( void* )table, class_count * sizeof( std::uintptr_t ) ) )
		return 0;
	for ( std::size_t i = 0; i < class_count; i++ )
		if ( *( std::uintptr_t* )( table + i * sizeof( std::uintptr_t ) ) == ( std::uintptr_t )klass )
			return table_rva + i * sizeof( std::uintptr_t );
	return 0;
}
...
{
	auto type_info_definition_table = c_scanner::resolve_type_info_definition_table( );
	...
	auto type_info_rva = [ & ]( il2cpp::il2cpp_class_t* klass ) -> std::uint64_t
	{
		auto rva = static_class_rva_exact( klass );
		if ( !rva )
			rva = type_definition_slot_rva( klass, type_info_definition_table );
		return rva;
	};
	...
	auto player_model = find_named( "PlayerModel" );
	auto model_state = find_model_state_class( );
	auto player_input = find_named( "PlayerInput" );
	auto base_projectile = find_named( "BaseProjectile" );
	auto held_entity = find_named( "HeldEntity" );
	auto base_view_model = find_named( "BaseViewModel" );
	auto auto_turret = find_named( "AutoTurret" );
	if ( !auto_turret )
		auto_turret = class_name_contains_i( "Auto", "Turret" );
	if ( !auto_turret )
		auto_turret = class_type_name_contains( "Auto", "Turret" );
	if ( !auto_turret )
		auto_turret = class_with_field_name_contains_type( "authorized", nullptr, false );
	auto player_corpse = find_named( "PlayerCorpse" );
	if ( !player_corpse )
		player_corpse = class_name_contains_i( "Player", "Corpse" );
	if ( !player_corpse )
		player_corpse = class_type_name_contains( "Player", "Corpse" );
	if ( !player_corpse )
		player_corpse = class_with_field_name_contains_type( "clientClothing", nullptr, false );
	auto lootable_corpse = find_named( "LootableCorpse" );
	if ( !lootable_corpse )
		lootable_corpse = class_name_contains_i( "Lootable", "Corpse" );
	if ( !lootable_corpse )
		lootable_corpse = class_type_name_contains( "Lootable", "Corpse" );
	if ( !lootable_corpse )
		lootable_corpse = class_with_field_names( "playerSteamID", "_playerName" );
	auto game_manager = find_game_manager_class( );
	auto game_manager_static = game_manager ? c_runtime::get_inner_static_class( game_manager ) : nullptr;
	...
	w.struct_field( "PlayerModel",           rva_of( player_model ) );
	...
	w.struct_field( "BaseNetworkable_TypeInfo", base_networkable_rva ? base_networkable_rva : class_rva( "BaseNetworkable" ) );
	w.struct_field( "BaseEntity_TypeInfo", rva_of( base_entity ) );
	w.struct_field( "BasePlayer_TypeInfo", rva_of( base_player ) );
	w.struct_field( "PlayerEyes_TypeInfo", rva_of( player_eyes ) );
	w.struct_field( "PlayerInventory_TypeInfo", rva_of( player_inventory ) );
	w.struct_field( "PlayerModel_TypeInfo", rva_of( player_model ) );
	w.struct_field( "ModelState_TypeInfo", type_info_rva( model_state ) );
	w.struct_field( "PlayerInput_TypeInfo", rva_of( player_input ) );
	w.struct_field( "BaseProjectile_TypeInfo", rva_of( base_projectile ) );
	w.struct_field( "HeldEntity_TypeInfo", rva_of( held_entity ) );
	w.struct_field( "BaseViewModel_TypeInfo", rva_of( base_view_model ) );
	w.struct_field( "AutoTurret_TypeInfo", rva_of( auto_turret ) );
	w.struct_field( "PlayerCorpse_TypeInfo", type_info_rva( player_corpse ) );
	w.struct_field( "LootableCorpse_TypeInfo", type_info_rva( lootable_corpse ) );
	w.struct_field( "GameManager_TypeInfo", type_info_rva( game_manager ) );
	w.struct_field( "GameManager_Static_TypeInfo", type_info_rva( game_manager_static ) );
	w.struct_end( );
}
...
{
	auto klass = find_game_manager_class( );
	...
	auto find_instance = [ klass ]( il2cpp::il2cpp_class_t* holder ) -> std::uint64_t
	{
		if ( !holder || !klass )
			return 0;
		void* iter = nullptr;
		while ( auto field = holder->fields( &iter ) )
		{
			if ( !( field->flags( ) & FIELD_ATTRIBUTE_STATIC ) && !holder->static_field_data( ) )
				continue;
			auto type = field->type( );
			if ( type && type->klass( ) == klass )
				return field->offset( holder );
		}
		return 0;
	};

	instance = find_instance( static_klass );
	if ( !instance && klass && klass->parent( ) )
	{
		auto parent_static = c_runtime::get_inner_static_class( klass->parent( ) );
		if ( auto parent_instance = find_instance( parent_static ) )
		{
			static_klass = parent_static;
			instance = parent_instance;
		}
	}
	auto rva = static_class_rva_any( static_klass );
	...
	w.struct_field( "static_fields", static_klass ? offsetof( Il2CppClass, static_fields ) : 0 );
	if ( instance )
...
void c_core::write_basenetworkable( c_writer& w )
{
	...
	auto named_parent_entity = field_offset_by_names( klass, parent_names, 4 );
	auto children = field_offset_by_names( klass, children_names, 2 );
	std::uint64_t typed_parent_entity = 0;
	if ( !typed_parent_entity )
		typed_parent_entity = first_entity_ref_field_after( klass, children );
	if ( !typed_parent_entity )
		typed_parent_entity = field_offset_by_type_or_class_contains( klass, "EntityRef" );
	auto parent_entity = typed_parent_entity ? typed_parent_entity : named_parent_entity;
	auto named_net = field_offset_by_names( klass, net_names, 3 );
	std::uint64_t typed_net = 0;
	if ( !typed_net )
		typed_net = field_offset_by_type_or_class_contains( klass, "Networkable" );
	if ( !typed_net )
		typed_net = field_offset_by_type_or_class_contains( base_entity, "Networkable" );
	auto net = typed_net ? typed_net : named_net;
	...
	if ( !parent_entity )
	{
		parent_entity = first_entity_ref_field_after( klass, children );
	}
	auto prefab_id = field_offset_by_name( klass, "prefabID" );
	if ( !prefab_id )
		prefab_id = field_offset_by_name_contains( klass, "prefab", "id" );
	if ( !prefab_id )
		prefab_id = field_offset_by_name( base_entity, "prefabID" );
	if ( !prefab_id )
		prefab_id = field_offset_by_name_contains( base_entity, "prefab", "id" );

	std::printf( "[morphine-dumper] base_networkable fields: parentEntity=0x%llX children=0x%llX net=0x%llX prefabID=0x%llX\n",
		parent_entity, children, net, prefab_id );
	...
	w.struct_field( "prefabID",             prefab_id );
...
void c_core::write_base_player( c_writer& w )
{
	...
	auto base_entity_klass   = c_runtime::find_class( "BaseEntity" );
	auto game_object_klass   = c_runtime::find_class( "GameObject", "UnityEngine" );
	if ( !game_object_klass )
		game_object_klass = find_named_class( "GameObject" );
	...
	auto model_state_klass   = find_model_state_class( );
	...
	if ( klass )
	{
		...
		{
			auto user_id_min = off_player_input ? off_player_input : off_cl_active_item;
			off_user_id = last_field_offset_between_type( klass, user_id_min, off_display_name, "UInt64" );
		}
		...
		if ( off_cl_active_item )
		{
			std::uint64_t wrapped_mounted = 0;
			void* iter = nullptr;
			while ( auto f = klass->fields( &iter ) )
			{
				...
				if ( off <= off_cl_active_item || ( off_user_id && off >= off_user_id ) )
					continue;

				auto type = f->type( );
				auto type_class = type ? type->klass( ) : nullptr;
				if ( !type_class || type_class == klass )
					continue;

				bool has_entity_at_zero = false;
				bool has_field_at_eight = false;
				int instance_fields = 0;
				void* inner_iter = nullptr;
				while ( auto inner = type_class->fields( &inner_iter ) )
				{
					if ( !inner || ( inner->flags( ) & FIELD_ATTRIBUTE_STATIC ) )
						continue;
					instance_fields++;
					auto inner_off = inner->offset( type_class );
					auto inner_type = inner->type( );
					auto inner_class = inner_type ? inner_type->klass( ) : nullptr;
					if ( inner_off == 0 &&
						( field_type_matches( inner, "BaseEntity" ) ||
							( base_entity_klass && class_is_or_inherits( inner_class, base_entity_klass ) ) ) )
						has_entity_at_zero = true;
					if ( inner_off == 0x8 )
						has_field_at_eight = true;
				}

				if ( has_entity_at_zero && has_field_at_eight && instance_fields == 2 )
				{
					if ( !wrapped_mounted || off < wrapped_mounted )
					{
						wrapped_mounted = off;
					}
				}
			...
			if ( wrapped_mounted )
				off_mounted = wrapped_mounted;
		}
		if ( off_mounted )
		{
			auto mounted_min = off_cl_active_item ? off_cl_active_item : off_player_input;
			auto mounted_max = off_user_id ? off_user_id : off_player_flags;
			if ( mounted_min && off_mounted <= mounted_min )
				off_mounted = 0;
			if ( mounted_max && off_mounted >= mounted_max )
				off_mounted = 0;
		}
		...
		if ( game_object_klass && off_model_state && off_player_model )
		{
			auto anchored_looking_at = field_offset_by_type_class_between(
				klass,
				game_object_klass,
				off_model_state,
				off_player_model );
			if ( anchored_looking_at )
				off_looking_at = anchored_looking_at;
		}
		if ( !off_looking_at )
			off_looking_at = field_offset_by_name_contains_type( klass, "looking", "at", "GameObject" );
		if ( !off_looking_at )
		...
		if ( !off_looking_at && base_entity_klass && off_model_state && off_player_model )
			off_looking_at = field_offset_by_type_class_between( klass, base_entity_klass, off_model_state, off_player_model );
	...
	std::printf(
		"[morphine-dumper] base_player resolved: currentTeam=0x%llX userID=0x%llX modelState=0x%llX mounted=0x%llX belt=0x%llX lookingAt=0x%llX\n",
		off_current_team,
		off_user_id,
		off_model_state,
		off_mounted,
	...
	w.struct_begin( "BasePlayer" );
	w.struct_field( "clActiveItem",      off_cl_active_item );
	w.struct_field( "PlayerEyes",        off_player_eyes );
	w.struct_field( "PlayerInventory",   off_player_inventory );
...
void c_core::write_base_combat_entity( c_writer& w )
{
	...
	w.struct_begin( "BaseCombatEntity" );
...
void c_core::write_base_entity( c_writer& w )
{
	...
	auto position_lerp = position_lerp_klass ? field_offset_by_type_class( klass, position_lerp_klass ) : 0;
	if ( !position_lerp )
		position_lerp = field_offset_by_name( klass, "positionLerp" );
		...
		for ( auto cur = klass; cur; cur = cur->parent( ) )
		{
			...
			while ( auto f = cur->fields( &iter ) )
			{
				if ( f->flags( ) & FIELD_ATTRIBUTE_STATIC )
					continue;
				auto off = f->offset( cur );
				if ( off <= flags || off > flags + 0x80 )
					continue;
				auto type = f->type( );
				auto type_name = type ? type->name( ) : nullptr;
				if ( !type_name ||
					ascii_contains_i( type_name, "Boolean" ) ||
					ascii_contains_i( type_name, "Int32" ) ||
					ascii_contains_i( type_name, "UInt32" ) ||
					ascii_contains_i( type_name, "Int64" ) ||
					ascii_contains_i( type_name, "UInt64" ) ||
					ascii_contains_i( type_name, "Single" ) ||
					ascii_contains_i( type_name, "String" ) )
					continue;
				if ( !best || off < best )
					best = off;
			}
...
void c_core::write_item_container( c_writer& w )
{
	...
	w.struct_begin( "ItemContainer" );
	w.struct_field( "ItemList", off_item_list );
...
static std::uint64_t first_field_offset_between_type(
	il2cpp::il2cpp_class_t* klass,
	std::uint64_t min_offset,
	std::uint64_t max_offset,
	const char* type )
{
	if ( !klass || !type || !max_offset || min_offset >= max_offset )
		return 0;
...
static std::uint64_t nth_field_offset_between_type(
	il2cpp::il2cpp_class_t* klass,
	std::uint64_t min_offset,
	std::uint64_t max_offset,
	const char* type,
	std::size_t index )
{
	...
	for ( std::size_t i = 0; i <= index; i++ )
	{
		picked = first_field_offset_between_type( klass, min_offset, max_offset, type );
		if ( !picked )
			return 0;
		min_offset = picked;
	}
...
static std::uint64_t last_field_offset_between_type(
	il2cpp::il2cpp_class_t* klass,
	std::uint64_t min_offset,
	std::uint64_t max_offset,
	const char* type )
{
	...
	for ( auto cur = klass; cur; cur = cur->parent( ) )
	{
		...
		while ( auto f = cur->fields( &iter ) )
		{
			...
			if ( off <= min_offset || off >= max_offset )
				continue;
			if ( field_type_or_generic_arg_matches( f, type ) && off > best )
				best = off;
		}
	}
	return best;
...
static std::uint64_t class_field_offset_by_name_contains( il2cpp::il2cpp_class_t* klass, const char* a, const char* b )
{
	if ( !klass || !a )
		return 0;
...
static bool is_entity_ref_type( il2cpp::il2cpp_class_t* klass )
{
	...
	while ( auto f = klass->fields( &iter ) )
	{
		...
		auto type_class = type ? type->klass( ) : nullptr;
		auto base_entity = c_runtime::find_class( "BaseEntity" );
		if ( base_entity && class_is_or_inherits( type_class, base_entity ) )
			return true;
	}
...
struct field_names_ctx_t
{
	const char* a;
	const char* b;
};

static bool field_names_predicate( il2cpp::field_info_t* field, void* raw_ctx )
{
	auto ctx = ( field_names_ctx_t* )raw_ctx;
	if ( !field || !ctx || !ctx->a )
		return false;
	auto name = field->name( );
	if ( !name || std::strcmp( name, ctx->a ) )
		return false;
	if ( !ctx->b )
		return true;

	auto owner = il2cpp::field_get_parent( field );
	if ( !owner )
		return false;
	return field_offset_by_name( owner, ctx->b ) != 0;
}

static il2cpp::il2cpp_class_t* class_with_field_names( const char* a, const char* b )
{
	field_names_ctx_t ctx{ a, b };
	return find_class_by_field( field_names_predicate, &ctx );
}
...
static il2cpp::il2cpp_class_t* find_model_state_class( )
{
	if ( auto klass = find_named_class( "ModelState" ) )
	{
		...
		if ( class_has_model_state_shape( klass ) || ( water_level && look_dir ) )
			return klass;
	}
...
void c_core::write_itemdefinition( c_writer& w )
{
	...
	auto write_fields = [&]( const char* name )
	{
		...
		auto item_mod_wearable = find_named_class( "ItemModWearable" );
		if ( !item_mod_wearable )
			item_mod_wearable = class_name_contains_i( "ItemMod", "Wearable" );

		auto item_mod_wearable_off = field_offset_by_type_class( klass, item_mod_wearable );
		if ( !item_mod_wearable_off )
			item_mod_wearable_off = field_offset_by_type_or_class_contains( klass, "ItemModWearable" );

		w.struct_field( "condition",       field_offset_by_name( klass, "condition" ) );
		w.struct_field( "ItemModWearable", item_mod_wearable_off );
		w.struct_end( );
	};
	...
	il2cpp::il2cpp_class_t* item_id_class )
{
	...
	w.struct_field( "HeldEntity",      held_entity );
	w.struct_end( );
}
...
void c_core::write_base_projectile( c_writer& w )
{
	...
	auto write_fields = [&]( const char* name )
	{
		...
		const char* fractional_reload_names[] = { "fractionalReload", "_fractionalReload" };
		const char* aimcone_curve_names[] = { "aimconeCurve", "aimConeCurve", "_aimconeCurve" };
		const char* no_aiming_while_cycling_names[] = { "noAimingWhileCycling", "_noAimingWhileCycling" };
		const char* hip_aim_cone_offset_names[] = { "hipAimConeOffset", "_hipAimConeOffset" };
		const char* sight_aim_cone_offset_names[] = { "sightAimConeOffset", "_sightAimConeOffset" };
		...
		auto fractional_reload = field_offset_by_names( klass, fractional_reload_names, 2 );
		if ( !fractional_reload )
			fractional_reload = field_offset_by_name_contains_type( klass, "fractional", "reload", "Boolean" );
		auto aimcone_curve = field_offset_by_names( klass, aimcone_curve_names, 3 );
		if ( !aimcone_curve )
			aimcone_curve = field_offset_by_name_contains( klass, "aimcone", "curve" );
		if ( !aimcone_curve )
			aimcone_curve = field_offset_by_name_contains( klass, "aimCone", "curve" );
		auto no_aiming_while_cycling = field_offset_by_names( klass, no_aiming_while_cycling_names, 2 );
		if ( !no_aiming_while_cycling )
			no_aiming_while_cycling = field_offset_by_name_contains_type( klass, "aiming", "cycling", "Boolean" );
		if ( !cached_mod_hash && no_aiming_while_cycling )
		{
			cached_mod_hash = first_field_offset_after_type( klass, no_aiming_while_cycling, "UInt32" );
			if ( !cached_mod_hash )
				cached_mod_hash = first_field_offset_after_type( klass, no_aiming_while_cycling, "Int32" );
		}
		auto hip_aim_cone_offset = field_offset_by_names( klass, hip_aim_cone_offset_names, 2 );
		if ( !hip_aim_cone_offset )
			hip_aim_cone_offset = field_offset_by_name_contains_type( klass, "hip", "offset", "Single" );
		auto sight_aim_cone_offset = field_offset_by_names( klass, sight_aim_cone_offset_names, 2 );
		if ( !sight_aim_cone_offset )
			sight_aim_cone_offset = field_offset_by_name_contains_type( klass, "sight", "offset", "Single" );
		if ( cached_mod_hash )
		{
			auto seq_sight_scale = first_field_offset_after_type( klass, cached_mod_hash, "Single" );
			auto seq_sight_offset = seq_sight_scale ? first_field_offset_after_type( klass, seq_sight_scale, "Single" ) : 0;
			auto seq_hip_scale = seq_sight_offset ? first_field_offset_after_type( klass, seq_sight_offset, "Single" ) : 0;
			auto seq_hip_offset = seq_hip_scale ? first_field_offset_after_type( klass, seq_hip_scale, "Single" ) : 0;
			if ( seq_sight_scale && ( !sight_aimcone_scale || sight_aimcone_scale <= cached_mod_hash ) )
				sight_aimcone_scale = seq_sight_scale;
			if ( seq_sight_offset && ( !sight_aim_cone_offset || sight_aim_cone_offset <= sight_aimcone_scale ) )
				sight_aim_cone_offset = seq_sight_offset;
			if ( seq_hip_scale && ( !hip_aim_cone_scale || hip_aim_cone_scale <= sight_aim_cone_offset ) )
				hip_aim_cone_scale = seq_hip_scale;
			if ( seq_hip_offset && ( !hip_aim_cone_offset || hip_aim_cone_offset <= hip_aim_cone_scale ) )
				hip_aim_cone_offset = seq_hip_offset;
		}
		...
		w.struct_field( "fractionalReload",        fractional_reload );
		w.struct_field( "aimconeCurve",            aimcone_curve );
		w.struct_field( "noAimingWhileCycling",    no_aiming_while_cycling );
		w.struct_field( "cachedModHash",           cached_mod_hash );
		w.struct_field( "hipAimConeOffset",        hip_aim_cone_offset );
		w.struct_field( "sightAimConeOffset",      sight_aim_cone_offset );
...
struct attack_layout_t
{
	il2cpp::il2cpp_class_t* klass = nullptr;
	std::uint64_t hit_bone = 0;
	std::uint64_t hit_position_world = 0;
	std::uint64_t hit_normal_local = 0;
	std::uint64_t hit_position_local = 0;
	std::uint64_t hit_part_id = 0;
};

static attack_layout_t resolve_attack_layout( )
{
	auto domain = il2cpp::il2cpp_domain_t::get( );
	if ( !domain )
		return { };
	...
	for ( size_t i = 0; i < assembly_count; i++ )
	{
		...
		for ( size_t j = 0; j < class_count; j++ )
		{
			...
			std::sort( fields.begin( ), fields.end( ), [ klass ]( il2cpp::field_info_t* lhs, il2cpp::field_info_t* rhs )
			{
				return lhs->offset( klass ) < rhs->offset( klass );
			} );

			for ( size_t k = 0; k + 13 < fields.size( ); k++ )
			{
				if ( !field_type_matches( fields[ k + 1 ], "Vector3" ) ||
					!field_type_matches( fields[ k + 2 ], "Boolean" ) ||
					!field_type_matches( fields[ k + 3 ], "Vector3" ) ||
					!field_type_matches( fields[ k + 4 ], "Boolean" ) ||
					!field_type_matches( fields[ k + 5 ], "Vector3" ) ||
					!field_type_matches( fields[ k + 6 ], "Vector3" ) ||
					!field_type_matches( fields[ k + 7 ], "UInt32" ) ||
					!field_type_matches( fields[ k + 9 ], "UInt32" ) ||
					!field_type_matches( fields[ k + 10 ], "Vector3" ) ||
					!field_type_matches( fields[ k + 12 ], "Vector3" ) ||
					!field_type_matches( fields[ k + 13 ], "UInt32" ) )
					continue;

				return {
					klass,
					fields[ k + 7 ]->offset( klass ),
					fields[ k + 1 ]->offset( klass ),
					fields[ k + 3 ]->offset( klass ),
					fields[ k + 5 ]->offset( klass ),
					fields[ k + 9 ]->offset( klass )
				};
			}
		}
	}
...
static void write_attack( c_writer& w )
{
	auto layout = resolve_attack_layout( );
	if ( !layout.klass )
		return;

	std::printf( "[morphine-dumper] attack: class=%p hitBone=0x%llX hitPositionWorld=0x%llX hitPartID=0x%llX\n",
		( void* )layout.klass,
		layout.hit_bone,
		layout.hit_position_world,
		layout.hit_part_id );

	w.struct_begin( "Attack" );
	w.struct_field( "hitBone",          layout.hit_bone );
	w.struct_field( "hitPositionWorld", layout.hit_position_world );
	w.struct_field( "hitNormalLocal",   layout.hit_normal_local );
	w.struct_field( "hitPositionLocal", layout.hit_position_local );
	w.struct_field( "hitPartID",        layout.hit_part_id );
	w.struct_end( );
}

static void write_base_melee( c_writer& w )
{
	auto klass = find_named_class( "BaseMelee" );
	auto damage_properties = field_offset_by_name( klass, "damageProperties" );
	if ( !damage_properties )
		damage_properties = field_offset_by_type_or_class_contains( klass, "DamageProperties" );
	auto gathering = field_offset_by_name( klass, "gathering" );
	if ( !gathering )
		gathering = field_offset_by_type_or_class_contains( klass, "GatherProperties" );

	w.struct_begin( "BaseMelee" );
	w.struct_field( "damageProperties",      damage_properties );
	w.struct_field( "maxDistance",           field_offset_by_name( klass, "maxDistance" ) );
	w.struct_field( "attackRadius",          field_offset_by_name( klass, "attackRadius" ) );
	w.struct_field( "blockSprintOnAttack",   field_offset_by_name( klass, "blockSprintOnAttack" ) );
	w.struct_field( "gathering",             gathering );
	w.struct_field( "canThrowAsProjectile",  field_offset_by_name( klass, "canThrowAsProjectile" ) );
	w.struct_end( );
}
...
static void write_player_input( c_writer& w )
{
	...
	{
		for ( auto cur = klass; cur && !state; cur = cur->parent( ) )
		{
			void* iter = nullptr;
			while ( auto field = cur->fields( &iter ) )
			{
				...
				if ( !type_class )
					continue;
				auto type_namespace = type_class->namespaze( );
				if ( ascii_contains_i( type_namespace, "System" ) ||
					ascii_contains_i( type_namespace, "UnityEngine" ) )
					continue;

				bool has_repeated_field_type = false;
				void* first_iter = nullptr;
				while ( auto first = type_class->fields( &first_iter ) )
				{
					if ( first->flags( ) & FIELD_ATTRIBUTE_STATIC )
						continue;
					auto first_type = first->type( );
					auto first_class = first_type ? first_type->klass( ) : nullptr;
					if ( !first_class )
						continue;

					void* second_iter = nullptr;
					while ( auto second = type_class->fields( &second_iter ) )
					{
						if ( second == first || ( second->flags( ) & FIELD_ATTRIBUTE_STATIC ) )
							continue;
						auto second_type = second->type( );
						if ( second_type && second_type->klass( ) == first_class )
						{
							has_repeated_field_type = true;
							break;
						}
					}
					if ( has_repeated_field_type )
						break;
				}
				if ( has_repeated_field_type )
				{
		...
		for ( auto cur = klass; cur; cur = cur->parent( ) )
		{
			...
			while ( auto f = cur->fields( &iter ) )
			{
				...
				if ( !type_name ||
					...
					ascii_contains_i( type_name, "UInt32" ) ||
					ascii_contains_i( type_name, "Single" ) ||
					ascii_contains_i( type_name, "Vector" ) ||
					ascii_contains_i( type_name, "String" ) )
					continue;
				candidate = off;
				count++;
	...
	std::printf( "[morphine-dumper] player_input: state=0x%llX bodyAngles=0x%llX yaw=0x%llX\n",
		state,
		body_angles,
		yaw );

	w.struct_begin( "PlayerInput" );
...
static void write_player_model( c_writer& w )
{
	...
	auto is_npc = g_base_player_live.player_model_is_npc;
	auto motion = resolve_player_model_motion_fields( klass, full_mask, skinned_multi_mesh );
	if ( motion.position )
		position = motion.position;
	if ( motion.new_velocity )
		velocity = motion.new_velocity;
	if ( !position && g_base_player_live.player_model_position )
		position = g_base_player_live.player_model_position;
	std::printf( "[morphine-dumper] player_model motion: position=0x%llX newVelocity=0x%llX\n",
		position,
		velocity );
	if ( !visible )
	{
...
{
	w.struct_begin( "BaseEntityFlags" );
...
static void write_model_state( c_writer& w )
{
		...
		for ( auto cur = klass; cur; cur = cur->parent( ) )
		{
			...
			while ( auto f = cur->fields( &iter ) )
			{
				if ( f->flags( ) & FIELD_ATTRIBUTE_STATIC )
					continue;
				auto off = f->offset( cur );
				if ( off <= water_level || ( look_dir && off >= look_dir ) )
					continue;
				auto type = f->type( );
				auto type_name = type ? type->name( ) : nullptr;
				if ( ascii_contains_i( type_name, "Single" ) ||
					...
					ascii_contains_i( type_name, "String" ) )
					continue;
				if ( !shaped_flags || off < shaped_flags )
					shaped_flags = off;
			}
		}
	...
	w.struct_begin( "ModelState" );
	w.struct_field( "flags",      flags );
	w.struct_field( "waterLevel", water_level );
...
static void write_held_entity( c_writer& w, il2cpp::il2cpp_class_t* item_id_class )
{
	...
	if ( !camera_punches )
	{
		for ( auto cur = klass; cur && !camera_punches; cur = cur->parent( ) )
		{
			void* iter = nullptr;
			while ( auto f = cur->fields( &iter ) )
			{
				...
				auto type = f->type( );
				auto type_class = type ? type->klass( ) : nullptr;
				auto element_class = type_class ? type_class->get_generic_argument_at( 0 ) : nullptr;
				if ( !element_class )
					continue;
				std::uint32_t vector_count = 0;
				std::uint32_t float_count = 0;
				std::uint32_t other_count = 0;
				void* element_iter = nullptr;
				while ( auto element_field = element_class->fields( &element_iter ) )
				{
					if ( element_field->flags( ) & FIELD_ATTRIBUTE_STATIC )
						continue;
					if ( field_type_matches( element_field, "Vector3" ) )
						vector_count++;
					else if ( field_type_matches( element_field, "Single" ) )
						float_count++;
					else
						other_count++;
				}
				if ( vector_count == 2 && float_count == 2 && other_count == 0 )
				{
					camera_punches = f->offset( cur );
					break;
				}
			}
		}
...
static void write_base_view_model( c_writer& w )
{
	...
	if ( !model )
		model = field_offset_by_type_or_class_contains( klass, "ViewmodelModel" );
	if ( !model )
		model = field_offset_by_type_or_class_contains( klass, "ViewModelModel" );
	...
	if ( !model && bob && lower )
	{
		for ( auto cur = klass; cur && !model; cur = cur->parent( ) )
		{
			void* iter = nullptr;
			while ( auto f = cur->fields( &iter ) )
			{
	...
	if ( !iron_sights )
		iron_sights = field_offset_by_type_or_class_contains( klass, "IronSights" );
	if ( !iron_sights )
		iron_sights = field_offset_by_type_or_class_contains( klass, "IronSight" );

	w.struct_begin( "BaseViewModel" );
...
static void write_chams( c_writer& w )
{
	auto player_model = find_named_class( "PlayerModel" );
	if ( !player_model )
		player_model = class_name_contains( "PlayerModel" );

	auto player_model_multi_mesh = field_offset_by_name( player_model, "_multiMesh" );
	if ( !player_model_multi_mesh )
		player_model_multi_mesh = field_offset_by_type_contains( player_model, "SkinnedMultiMesh" );

	auto skinned_multi_mesh = find_named_class( "SkinnedMultiMesh" );
	auto multi_mesh_renderer_list = field_offset_by_name( skinned_multi_mesh, "Renderers" );
	if ( !multi_mesh_renderer_list )
		multi_mesh_renderer_list = field_offset_by_name( skinned_multi_mesh, "RendererList" );
	if ( !multi_mesh_renderer_list )
		multi_mesh_renderer_list = field_offset_by_name_contains( skinned_multi_mesh, "renderer" );
	if ( !multi_mesh_renderer_list )
		multi_mesh_renderer_list = field_offset_by_type_contains( skinned_multi_mesh, "Renderer" );

	w.struct_begin( "chams" );
	w.struct_field( "player_model_multi_mesh", player_model_multi_mesh );
	w.struct_field( "multi_mesh_renderer_list", multi_mesh_renderer_list );
	w.struct_end( );
}
...
static void write_auto_turret( c_writer& w )
{
	...
	if ( !klass )
		klass = class_type_name_contains( "Auto", "Turret" );
	if ( !klass )
		klass = class_with_field_name_contains_type( "authorized", nullptr, false );

	auto authorized_players = class_field_offset_by_name_contains( klass, "authorized", "players" );
	if ( !authorized_players )
		authorized_players = field_offset_by_name_contains_type( klass, "authorized", "players", "HashSet" );
	auto typed_authorized_players = field_offset_by_type_contains( klass, "HashSet<System.UInt64>" );
	if ( !typed_authorized_players )
		typed_authorized_players = field_offset_by_type_contains( klass, "HashSet<ulong>" );
	if ( typed_authorized_players )
		authorized_players = typed_authorized_players;
	if ( !authorized_players )
	{
		for ( auto cur = klass; cur && !authorized_players; cur = cur->parent( ) )
		{
			void* iter = nullptr;
			while ( auto f = cur->fields( &iter ) )
			{
				...
				auto type = f->type( );
				auto type_name = type ? type->name( ) : nullptr;
				if ( ascii_contains_i( type_name, "HashSet" ) &&
					field_type_or_generic_arg_matches( f, "UInt64" ) )
				{
					authorized_players = f->offset( cur );
					break;
				}
			}
		}
	...
	auto last_yaw = class_field_offset_by_name_contains( klass, "last", "yaw" );
	if ( !last_yaw )
		last_yaw = field_offset_by_name_contains_type( klass, "last", "yaw", "Quaternion" );
	auto quaternion_yaw = field_offset_by_type_contains_after( klass, "Quaternion", authorized_players );
	if ( quaternion_yaw && ( !muzzle_pos || quaternion_yaw < muzzle_pos ) )
		last_yaw = quaternion_yaw;

	std::printf( "[morphine-dumper] auto_turret: authorizedPlayers=0x%llX lastYaw=0x%llX muzzlePos=0x%llX\n",
		authorized_players,
		last_yaw,
		muzzle_pos );
...
{
	auto klass = class_with_field_names( "bonePosData", "underwearSkin" );
	if ( !klass )
		klass = find_named_class( "PlayerCorpse" );
	if ( !klass )
		klass = class_name_contains_i( "Player", "Corpse" );
	if ( !klass )
		klass = class_type_name_contains( "Player", "Corpse" );
	if ( !klass )
		klass = class_with_field_name_contains_type( "clientClothing", nullptr, false );
	if ( !klass )
		klass = class_with_field_name_contains_type( "client", nullptr, false );
	...
	if ( !client_clothing )
		client_clothing = field_offset_by_name_contains_type( klass, "client", "clothing", "List" );
	if ( !client_clothing )
	{
		for ( auto cur = klass; cur && !client_clothing; cur = cur->parent( ) )
		{
			void* iter = nullptr;
			while ( auto f = cur->fields( &iter ) )
			{
				if ( f->flags( ) & FIELD_ATTRIBUTE_STATIC )
					continue;
				if ( field_type_or_generic_arg_matches( f, "Clothing" ) ||
					field_type_or_generic_arg_matches( f, "Wearable" ) )
				{
					client_clothing = f->offset( cur );
					break;
				}
			}
		}
	}
	auto bone_pos_data = field_offset_by_name( klass, "bonePosData" );
	auto ragdoll = field_offset_by_type_contains_after( klass, "Ragdoll", bone_pos_data );
	if ( bone_pos_data && ragdoll )
	{
		std::uint64_t anchored_clothing = 0;
		void* iter = nullptr;
		while ( auto f = klass->fields( &iter ) )
		{
			if ( !f || ( f->flags( ) & FIELD_ATTRIBUTE_STATIC ) )
				continue;
			auto off = f->offset( klass );
			if ( off > bone_pos_data && off < ragdoll &&
				( !anchored_clothing || off < anchored_clothing ) )
				anchored_clothing = off;
		}
		if ( anchored_clothing )
			client_clothing = anchored_clothing;
	}

	std::printf( "[morphine-dumper] player_corpse: clientClothing=0x%llX\n", client_clothing );
...
static void write_lootable_corpse( c_writer& w )
{
	auto player_corpse = class_with_field_names( "bonePosData", "underwearSkin" );
	auto klass = player_corpse ? player_corpse->parent( ) : nullptr;
	if ( !klass )
		klass = find_named_class( "LootableCorpse" );
	if ( !klass )
		klass = class_name_contains_i( "Lootable", "Corpse" );
	if ( !klass )
		klass = class_type_name_contains( "Lootable", "Corpse" );
	if ( !klass )
		klass = class_with_field_names( "playerSteamID", "_playerName" );
	if ( !klass )
		klass = class_with_field_name_contains_type( "playerSteamID", "UInt64", false );

	auto player_steam_id = field_offset_by_name( klass, "playerSteamID" );
	if ( !player_steam_id )
		player_steam_id = field_offset_by_name_contains_type( klass, "steam", nullptr, "UInt64" );
	if ( !player_steam_id )
		player_steam_id = field_offset_by_name_contains_type( klass, "steam", nullptr, "ulong" );
	if ( !player_steam_id )
		player_steam_id = getter_field_offset_by_return_type_name( klass, "UInt64", "steam" );
	auto loot_panel_name = field_offset_by_name( klass, "lootPanelName" );
	if ( loot_panel_name )
	{
		auto anchored_steam_id = first_field_offset_after_type( klass, loot_panel_name, "UInt64" );
		if ( anchored_steam_id )
			player_steam_id = anchored_steam_id;
	}

	auto player_name = field_offset_by_name( klass, "_playerName" );
	if ( !player_name )
		player_name = field_offset_by_name( klass, "playerName" );
	if ( !player_name )
		player_name = field_offset_by_name_contains_type( klass, "player", "name", "String" );
	if ( player_steam_id )
	{
		auto anchored_player_name = first_field_offset_after_type( klass, player_steam_id, "String" );
		if ( anchored_player_name )
			player_name = anchored_player_name;
	}

	std::printf(
		"[morphine-dumper] lootable_corpse: playerSteamID=0x%llX playerName=0x%llX\n",
		player_steam_id,
		player_name );

	w.struct_begin( "LootableCorpse" );
	w.struct_field( "playerSteamID", player_steam_id );
	w.struct_field( "_playerName",   player_name );
	w.struct_end( );
}
...
static void write_tod_sky( c_writer& w )
{
	...
	w.struct_begin( "TOD_CycleParameters" );
...
static void write_unity_constants( c_writer& w )
{
	...
	auto object_klass = c_runtime::find_class( "Object", "UnityEngine" );
	auto cached_ptr = field_offset_by_name( object_klass, "m_CachedPtr" );
	if ( !cached_ptr )
		cached_ptr = field_offset_by_name_contains( object_klass, "CachedPtr" );

	w.struct_begin( "Object" );
	w.struct_field( "m_CachedPtr", cached_ptr );
	w.struct_end( );

	w.struct_begin( "unity_object" );
	w.struct_field( "cached_ptr", cached_ptr );
	w.struct_end( );
...
static bool enum_name_exists( const std::vector<std::string>& used, const std::string& name )
{
...
static std::string bone_enum_name( const std::string& raw, int index, std::vector<std::string>& used )
{
	std::string out;
	...
	auto base = out;
	auto unique = base;
	for ( int n = 0; enum_name_exists( used, unique ); n++ )
	{
		char suffix[ 32 ]{ };
		std::snprintf( suffix, sizeof( suffix ), n ? "_%d_%d" : "_%d", index, n );
		unique = base + suffix;
	}
...
static bool write_bone_enums( const char* output_path, const std::vector<std::string>& bone_names )
{
	if ( !output_path || bone_names.empty( ) )
		return false;
	...
	std::fprintf( file, "enum class BoneList : int\n{\n" );

	std::vector<std::string> used;
	for ( std::size_t i = 0; i < bone_names.size( ); i++ )
	{
		auto name = bone_enum_name( bone_names[ i ], ( int )i, used );
		std::fprintf( file, "\t%s = %zu%s\n",
			name.c_str( ),
			i,
			i + 1 < bone_names.size( ) ? "," : "" );
	}
...
struct prefab_entry_t
{
	std::uint32_t id;
	std::string path;
};

struct il2cpp_array_header_t
{
	std::uintptr_t klass;
	std::uintptr_t monitor;
	std::uintptr_t bounds;
	il2cpp_array_size_t length;
};

static_assert( sizeof( il2cpp_array_header_t ) == 0x20 );

static il2cpp::field_info_t* exact_field(
	il2cpp::il2cpp_class_t* klass,
	const char* name,
	bool require_static )
{
	...
	while ( auto field = klass->fields( &iter ) )
	{
		if ( !!( field->flags( ) & FIELD_ATTRIBUTE_STATIC ) != require_static )
			continue;
		auto field_name = field->name( );
		if ( field_name && !_stricmp( field_name, name ) )
			return field;
	}
	return nullptr;
}

static std::uintptr_t static_pointer_value( il2cpp::field_info_t* field )
{
	if ( !field )
		return 0;
	...
	__try
	{
		return ( std::uintptr_t )field->static_get_value<void*>( );
	}
	__except ( EXCEPTION_EXECUTE_HANDLER )
	{
		return 0;
...
static bool prefab_field_type_is( il2cpp::field_info_t* field, const char* full_name, const char* short_name )
{
	auto type = field ? field->type( ) : nullptr;
	auto name = type ? type->name( ) : nullptr;
	return name && ( !std::strcmp( name, full_name ) || !std::strcmp( name, short_name ) );
}

static bool collect_runtime_prefabs( std::vector<prefab_entry_t>& entries )
{
	auto manifest_class = c_runtime::find_class( "GameManifest" );
	if ( !manifest_class )
	{
		std::printf( "[morphine-dumper] prefab ids: GameManifest class not found\n" );
		return false;
	}

	auto current_field = exact_field( manifest_class, "Current", true );
	if ( !current_field )
	{
		void* iter = nullptr;
		while ( auto field = manifest_class->fields( &iter ) )
		{
			if ( !( field->flags( ) & FIELD_ATTRIBUTE_STATIC ) )
				continue;
			auto type = field->type( );
			if ( type && type->klass( ) == manifest_class )
			{
				if ( current_field )
					return false;
				current_field = field;
	...
	auto manifest = static_pointer_value( current_field );
	auto find_manifest_instance = [&]( il2cpp::il2cpp_class_t* holder ) -> std::uintptr_t
	{
		if ( !holder )
			return 0;

		auto static_data = ( std::uintptr_t )holder->static_field_data( );
		void* iter = nullptr;
		while ( auto field = holder->fields( &iter ) )
		{
			auto type = field->type( );
			...
			auto value = field->flags( ) & FIELD_ATTRIBUTE_STATIC
				? static_pointer_value( field )
				: static_data
					? read_ptr_safe( static_data + field->offset( holder ) )
					: 0;
			if ( is_valid_userland_ptr( value ) && object_is_class( value, manifest_class ) )
				return value;
		}
		return 0;
	};
	if ( !is_valid_userland_ptr( manifest ) || !object_is_class( manifest, manifest_class ) )
		manifest = find_manifest_instance( manifest_class );
	if ( !manifest )
	{
		void* nested_iter = nullptr;
		while ( auto nested = manifest_class->nested_types( &nested_iter ) )
		{
			manifest = find_manifest_instance( nested );
			if ( manifest )
				break;
		}
	}
	if ( !is_valid_userland_ptr( manifest ) || !object_is_class( manifest, manifest_class ) )
	{
		std::printf( "[morphine-dumper] prefab ids: GameManifest.Current is unavailable\n" );
		return false;
	}

	auto pooled_field = exact_field( manifest_class, "pooledStrings", false );
	if ( !pooled_field )
	{
		std::size_t pooled_matches = 0;
		void* pooled_iter = nullptr;
		while ( auto field = manifest_class->fields( &pooled_iter ) )
		{
			if ( field->flags( ) & FIELD_ATTRIBUTE_STATIC )
				continue;
			auto type = field->type( );
			auto array_class_candidate = type ? type->klass( ) : nullptr;
			auto native_array_class_candidate = ( Il2CppClass* )array_class_candidate;
			auto element_class_candidate = native_array_class_candidate
				? ( il2cpp::il2cpp_class_t* )native_array_class_candidate->element_class
				: nullptr;
			if ( !array_class_candidate || !element_class_candidate )
				continue;

			std::size_t hash_fields = 0;
			std::size_t path_fields = 0;
			void* element_iter = nullptr;
			while ( auto element_field = element_class_candidate->fields( &element_iter ) )
			{
				if ( element_field->flags( ) & FIELD_ATTRIBUTE_STATIC )
					continue;
				if ( prefab_field_type_is( element_field, "System.UInt32", "UInt32" ) )
					++hash_fields;
				else if ( prefab_field_type_is( element_field, "System.String", "String" ) )
					++path_fields;
			}
			if ( hash_fields == 1 && path_fields == 1 )
			{
				pooled_field = field;
				++pooled_matches;
			}
		...
		if ( pooled_matches > 1 )
		{
			std::printf( "[morphine-dumper] prefab ids: pooledStrings field is ambiguous\n" );
			return false;
		}
		if ( !pooled_field )
		{
			std::printf( "[morphine-dumper] prefab ids: no pooledStrings-shaped field found\n" );
			return false;
		}
	}
	auto array_class = pooled_field && pooled_field->type( )
		? pooled_field->type( )->klass( ) : nullptr;
	auto native_array_class = ( Il2CppClass* )array_class;
	auto element_class = native_array_class
		? ( il2cpp::il2cpp_class_t* )native_array_class->element_class : nullptr;
	auto native_element_class = ( Il2CppClass* )element_class;
	if ( !pooled_field || !array_class || !element_class || !native_element_class )
	{
		std::printf( "[morphine-dumper] prefab ids: pooledStrings metadata is unavailable field=%p array_class=%p element=%p native_element=%p\n",
			pooled_field,
			array_class,
			element_class,
			native_element_class );
		return false;
	}

	il2cpp::field_info_t* hash_field = nullptr;
	il2cpp::field_info_t* path_field = nullptr;
	auto resolve_element_shape = [&]( il2cpp::il2cpp_class_t* candidate_element_class ) -> bool
	{
		hash_field = nullptr;
		path_field = nullptr;
		std::size_t hash_field_count = 0;
		std::size_t path_field_count = 0;
		void* iter = nullptr;
		while ( auto field = candidate_element_class->fields( &iter ) )
		{
			if ( field->flags( ) & FIELD_ATTRIBUTE_STATIC )
				continue;
			if ( prefab_field_type_is( field, "System.UInt32", "UInt32" ) )
			{
				hash_field = field;
				++hash_field_count;
			}
			else if ( prefab_field_type_is( field, "System.String", "String" ) )
			{
				path_field = field;
				++path_field_count;
			}
		}
		return hash_field_count == 1 && path_field_count == 1;
	};
	if ( !resolve_element_shape( element_class ) )
	{
		std::printf( "[morphine-dumper] prefab ids: PooledString hash/path fields are unresolved element=%p\n",
			element_class );
		return false;
	}

	auto pooled_array = read_ptr_safe( manifest + pooled_field->offset( manifest_class ) );
	auto pooled_array_is_readable = []( std::uintptr_t array ) -> bool
	{
		constexpr auto length_offset = offsetof( il2cpp_array_header_t, length );
		if ( !is_valid_userland_ptr( array + length_offset ) ||
			IsBadReadPtr( ( void* )( array + length_offset ), sizeof( il2cpp_array_size_t ) ) )
			return false;
		auto count = *( il2cpp_array_size_t* )( array + length_offset );
		return count > 0 && count <= 1000000;
	};
	if ( !pooled_array || ( !object_is_class( pooled_array, array_class ) && !pooled_array_is_readable( pooled_array ) ) )
	{
		void* live_iter = nullptr;
		while ( auto field = manifest_class->fields( &live_iter ) )
		{
			if ( field->flags( ) & FIELD_ATTRIBUTE_STATIC )
				continue;

			auto type = field->type( );
			auto candidate_array_class = type ? type->klass( ) : nullptr;
			auto candidate_native_array_class = ( Il2CppClass* )candidate_array_class;
			auto candidate_element_class = candidate_native_array_class
				? ( il2cpp::il2cpp_class_t* )candidate_native_array_class->element_class
				: nullptr;
			auto candidate_native_element_class = ( Il2CppClass* )candidate_element_class;
			if ( !candidate_array_class || !candidate_element_class || !candidate_native_element_class )
				continue;
			if ( !resolve_element_shape( candidate_element_class ) )
				continue;

			auto candidate_array = read_ptr_safe( manifest + field->offset( manifest_class ) );
			if ( !candidate_array ||
				( !object_is_class( candidate_array, candidate_array_class ) && !pooled_array_is_readable( candidate_array ) ) )
				continue;

			pooled_field = field;
			array_class = candidate_array_class;
			native_array_class = candidate_native_array_class;
			element_class = candidate_element_class;
			native_element_class = candidate_native_element_class;
			pooled_array = candidate_array;
			break;
		}
		if ( !pooled_array || ( !object_is_class( pooled_array, array_class ) && !pooled_array_is_readable( pooled_array ) ) )
		{
			std::printf( "[morphine-dumper] prefab ids: GameManifest.pooledStrings is unavailable\n" );
			return false;
		}
	}

	constexpr auto length_offset = offsetof( il2cpp_array_header_t, length );
	constexpr auto data_offset = sizeof( il2cpp_array_header_t );
	if ( IsBadReadPtr( ( void* )( pooled_array + length_offset ), sizeof( il2cpp_array_size_t ) ) )
	{
		std::printf( "[morphine-dumper] prefab ids: pooledStrings length is unreadable array=%p field=0x%llX\n",
			( void* )pooled_array,
			pooled_field->offset( manifest_class ) );
		return false;
	}
	auto count = *( il2cpp_array_size_t* )( pooled_array + length_offset );
	if ( !count || count > 1000000 )
	{
		std::printf( "[morphine-dumper] prefab ids: invalid pooledStrings count=%zu array=%p field=0x%llX\n",
			( std::size_t )count,
			( void* )pooled_array,
			pooled_field->offset( manifest_class ) );
		return false;
	...
	auto hash_offset = hash_field->offset( element_class );
	auto path_offset = path_field->offset( element_class );
	auto value_type = native_element_class->valuetype;
	auto stride = native_array_class->element_size;
	if ( !stride )
	{
		std::printf( "[morphine-dumper] prefab ids: invalid pooledStrings stride array_class=%p element=%p\n",
			array_class,
			element_class );
		return false;
	}
	...
	{
		auto slot = pooled_array + data_offset + i * stride;
		auto entry = value_type ? slot : read_ptr_safe( slot );
		if ( !entry ||
			IsBadReadPtr( ( void* )( entry + hash_offset ), sizeof( std::uint32_t ) ) )
			continue;

		auto id = *( std::uint32_t* )( entry + hash_offset );
		auto path = read_ptr_safe( entry + path_offset );
		std::string asset_path;
		if ( !id || !managed_string_to_ascii( path, asset_path ) || asset_path.empty( ) )
			continue;
		entries.push_back( prefab_entry_t{ id, std::move( asset_path ) } );
	}

	std::printf(
		"[morphine-dumper] prefab ids runtime: manifest=%p array=%p count=%zu extracted=%zu stride=0x%zX\n",
		( void* )manifest,
		( void* )pooled_array,
		( std::size_t )count,
		entries.size( ),
		stride );
	if ( entries.empty( ) )
		std::printf( "[morphine-dumper] prefab ids: no readable prefab paths hash_off=0x%llX path_off=0x%llX value_type=%d\n",
			hash_offset,
			path_offset,
			value_type ? 1 : 0 );
...
static bool collect_live_prefab_ids(
	std::vector<prefab_entry_t>& entries,
	std::uint64_t base_networkable_rva,
	std::uint32_t bn_static_fields,
	std::uint32_t wrapper_class_ptr,
	std::uint32_t parent_static_fields,
	...
	const c_decryption::constants_t& decrypt_1 )
{
	if ( !base_networkable_rva || !wrapper_class_ptr || !parent_static_fields || !entities ||
		!hv_offset || !decrypt_0.valid || !decrypt_1.valid )
		return false;

	auto base_networkable = c_runtime::find_class( "BaseNetworkable" );
	auto base_entity = c_runtime::find_class( "BaseEntity" );
	auto prefab_id_offset = field_offset_by_name( base_networkable, "prefabID" );
	if ( !prefab_id_offset )
		prefab_id_offset = field_offset_by_name_contains( base_networkable, "prefab", "id" );
	if ( !prefab_id_offset )
		prefab_id_offset = field_offset_by_name( base_entity, "prefabID" );
	if ( !prefab_id_offset )
		prefab_id_offset = field_offset_by_name_contains( base_entity, "prefab", "id" );
	if ( !prefab_id_offset )
	{
		std::printf( "[morphine-dumper] prefab ids live: prefabID offset unresolved\n" );
		return false;
	}
	...
	auto entity_class = base_networkable ? base_networkable : base_entity;
	if ( !entity_class )
		return false;

	std::vector<std::uint32_t> ids;
	auto limit = count > 4096 ? 4096 : count;
	ids.reserve( ( std::size_t )limit );
	for ( std::int32_t i = 0; i < limit; i++ )
	{
		auto entry = *( std::uintptr_t* )( array + 0x20 + ( i * 8 ) );
		auto entity = resolve_entity_entry_object( entry, entity_class );
		if ( !is_valid_userland_ptr( entity + prefab_id_offset ) )
			continue;
		auto id = *( std::uint32_t* )( entity + prefab_id_offset );
		if ( id )
			ids.push_back( id );
	}
	...
	for ( auto id : ids )
		entries.push_back( prefab_entry_t{ id, std::string{ } } );

	std::printf( "[morphine-dumper] prefab ids live: prefabID=0x%llX entities=%d unique=%zu\n",
		prefab_id_offset,
		count,
		entries.size( ) );
	return true;
...
static void write_cpp_string( std::FILE* file, const std::string& value )
{
	std::fputc( '"', file );
	for ( auto ch : value )
	{
...
static bool write_prefab_entries( const char* output_path, const std::vector<prefab_entry_t>& entries, std::size_t& count )
{
	count = 0;
	if ( !output_path || entries.empty( ) )
		return false;
	...
	std::fprintf(
		...
		"\t\tconst char* path;\n"
		"\t};\n\n"
		"\tinline constexpr entry_t entries[] =\n"
		"\t{\n" );

	for ( const auto& entry : entries )
	{
		std::fprintf( file, "\t\t{ %uu, ", entry.id );
		write_cpp_string( file, entry.path );
		std::fprintf( file, " },\n" );
	}
	std::fprintf(
		file,
		"\t};\n\n"
		"\tinline constexpr std::size_t count = sizeof( entries ) / sizeof( entries[ 0 ] );\n"
		"}\n" );

	auto written = !std::ferror( file );
	std::fclose( file );
	count = written ? entries.size( ) : 0;
	return written;
}

static bool write_prefab_ids( const char* output_path, std::size_t& count )
{
	std::vector<prefab_entry_t> entries;
	if ( !collect_runtime_prefabs( entries ) )
		return false;
	return write_prefab_entries( output_path, entries, count );
}
...
struct effect_network_classes_t
{
	il2cpp::il2cpp_class_t* owner = nullptr;
	il2cpp::il2cpp_class_t* statics = nullptr;
	il2cpp::il2cpp_class_t* effect = nullptr;
};

static bool is_effect_payload_class( il2cpp::il2cpp_class_t* klass )
{
	if ( !klass )
		return false;
	std::uint64_t string_offset = 0;
	std::vector<std::uint64_t> vector_offsets;
	...
	while ( auto field = klass->fields( &iter ) )
	{
		if ( field->flags( ) & FIELD_ATTRIBUTE_STATIC )
			continue;
		auto offset = field->offset( klass );
		if ( field_type_matches( field, "String" ) )
			string_offset = offset;
		else if ( field_type_matches( field, "Vector3" ) )
			vector_offsets.push_back( offset );
		else if ( field_type_matches( field, "Transform" ) )
			transform_count++;
		else if ( field_type_matches( field, "GameObject" ) )
			game_object_count++;
	}
	std::sort( vector_offsets.begin( ), vector_offsets.end( ) );
	auto vector_after_string = std::any_of( vector_offsets.begin( ), vector_offsets.end( ),
		[ string_offset ]( std::uint64_t offset ) { return offset > string_offset; } );
	return string_offset &&
		vector_offsets.size( ) == 3 &&
		transform_count == 1 &&
		game_object_count == 1 &&
		vector_after_string;
}

static effect_network_classes_t find_effect_network_classes( )
{
	effect_network_classes_t result{};
	auto domain = il2cpp::il2cpp_domain_t::get( );
	size_t assembly_count = 0;
	auto assemblies = domain ? domain->get_assemblies( &assembly_count ) : nullptr;
	for ( size_t i = 0; assemblies && i < assembly_count; i++ )
	{
		...
		for ( size_t j = 0; j < class_count; j++ )
		{
			auto owner = image->get_class( j );
			if ( !owner )
				continue;
			void* nested_iter = nullptr;
			while ( auto nested = owner->nested_types( &nested_iter ) )
			{
				il2cpp::il2cpp_class_t* payload = nullptr;
				...
				while ( auto field = nested->fields( &field_iter ) )
				{
					...
					if ( !payload )
						payload = field_class;
					else if ( field_class != payload )
						same_payload = false;
					static_count++;
				}
				if ( static_count == 2 && same_payload && is_effect_payload_class( payload ) &&
					static_class_rva_any( owner ) && static_class_rva_any( nested ) )
				{
					result.owner = owner;
					result.statics = nested;
...
{
	auto classes = find_effect_network_classes( );
	if ( !classes.owner || !classes.statics || !classes.effect )
		return;
	auto type_info = static_class_rva_exact( classes.owner );
	if ( !type_info )
		type_info = type_definition_slot_rva( classes.owner, c_scanner::resolve_type_info_definition_table( ) );
	auto static_type_info = static_class_rva_exact( classes.statics );
	if ( !static_type_info )
		static_type_info = type_definition_slot_rva( classes.statics, c_scanner::resolve_type_info_definition_table( ) );
	std::uint64_t instance = 0;
	if ( classes.statics )
	{
		void* iter = nullptr;
		while ( auto field = classes.statics->fields( &iter ) )
		{
			if ( !( field->flags( ) & FIELD_ATTRIBUTE_STATIC ) )
				continue;
			auto type = field->type( );
			if ( type && type->klass( ) == classes.effect )
			{
				auto offset = ( std::uint64_t )field->offset( classes.statics );
				if ( offset > instance )
					instance = offset;
			}
		}
	}
	std::uint64_t hit_position = 0;
	std::uint64_t string_offset = 0;
	std::vector<std::uint64_t> vector_offsets;
	if ( classes.effect )
	{
		void* iter = nullptr;
		while ( auto field = classes.effect->fields( &iter ) )
		{
			if ( field->flags( ) & FIELD_ATTRIBUTE_STATIC )
				continue;
			auto offset = field->offset( classes.effect );
			if ( field_type_matches( field, "String" ) )
				string_offset = offset;
			else if ( field_type_matches( field, "Vector3" ) )
				vector_offsets.push_back( offset );
		}
		for ( auto offset : vector_offsets )
			if ( offset > string_offset && ( !hit_position || offset < hit_position ) )
				hit_position = offset;
	}
	if ( !type_info || !static_type_info || !instance || !hit_position )
		return;

	auto class_name = classes.owner ? classes.owner->name( ) : nullptr;
	auto class_ns = classes.owner ? classes.owner->namespaze( ) : nullptr;
	auto class_type = classes.owner ? classes.owner->type( ) : nullptr;
	auto class_type_name = class_type ? class_type->name( ) : nullptr;
	auto nested_name = classes.statics ? classes.statics->name( ) : nullptr;
	auto nested_type = classes.statics ? classes.statics->type( ) : nullptr;
	...
	w.struct_begin( "EffectNetwork" );
	w.struct_field( "type_info", type_info );
	w.struct_field( "static_type_info", static_type_info );
	w.struct_field( "static_fields", classes.statics ? offsetof( Il2CppClass, static_fields ) : 0 );
	w.struct_field( "instance", instance );
...
static void write_convar_admin( c_writer& w )
{
	...
	std::printf( "[morphine-dumper] convar admin: class=%p rva=0x%llX playerIds=0x%llX\n",
		admin, admin_rva, player_ids );
...
void c_core::write_decryptions( c_writer& w )
{
	...
	if ( m_cl_active_item_plain )
		w.write_identity_u32_fn( "clActiveItem" );
	else if ( m_cl_active_item_constants.valid )
		w.write_direct_decryption_fn_single(
			"clActiveItem",
			m_cl_active_item_fn_addr - c_runtime::game_base( ),
			m_cl_active_item_constants );
	else
		w.write_decryption_unresolved( "clActiveItem" );
	if ( m_player_inventory_constants.valid )
		...
		w.write_decryption_unresolved( "PlayerInventory" );
	if ( m_player_eyes_constants.valid )
		...
		w.write_decryption_unresolved( "PlayerEyes" );
...
bool c_core::run( const char* output_path, bool do_materials )
{
	...
	auto decryptions_distinct =
		!m_player_inventory_constants.valid ||
		!m_player_eyes_constants.valid ||
		!c_decryption::ops_match( m_player_inventory_constants, m_player_eyes_constants );
	if ( !decryptions_distinct )
	{
		m_player_eyes_fn_addr = 0;
		g_player_eyes_hv_offset = 0;
		m_player_eyes_constants = { };
		g_player_eyes_verified = false;
	}
	std::printf(
		"[morphine-dumper] decryption verification: clActiveItem=%s PlayerInventory=%s PlayerEyes=%s distinct=%s\n",
		g_cl_active_item_verified ? "PASS" : "FAIL",
		g_player_inventory_verified ? "PASS" : "FAIL",
		g_player_eyes_verified ? "PASS" : "FAIL",
		decryptions_distinct ? "PASS" : "FAIL" );
	resolve_model_bones( );
	...
	write_il2cpp_api( w, m_gc_handles_rva );
	...
	if ( !m_model_bone_names.empty( ) )
	{
		char enum_path[ MAX_PATH ]{ };
		std::strncpy( enum_path, output_path, MAX_PATH - 1 );
		if ( auto slash = std::strrchr( enum_path, '\\' ) )
			std::snprintf( slash + 1, MAX_PATH - ( slash - enum_path + 1 ), "enums.h" );
		else
			std::snprintf( enum_path, MAX_PATH, "enums.h" );

		std::printf( "[morphine-dumper] bone enums output: %s\n", enum_path );
		if ( write_bone_enums( enum_path, m_model_bone_names ) )
			std::printf( "[morphine-dumper] bone enums written to %s (%zu entries)\n",
				enum_path, m_model_bone_names.size( ) );
		else
			std::printf( "[morphine-dumper] bone enums failed to write\n" );
	}
	else
	{
		std::printf( "[morphine-dumper] bone enums: no bone names extracted\n" );
	}

	char prefab_path[ MAX_PATH ]{ };
	std::strncpy( prefab_path, output_path, MAX_PATH - 1 );
	if ( auto slash = std::strrchr( prefab_path, '\\' ) )
		std::snprintf( slash + 1, MAX_PATH - ( slash - prefab_path + 1 ), "prefabs.h" );
	else
		std::snprintf( prefab_path, MAX_PATH, "prefabs.h" );

	std::printf( "[morphine-dumper] prefab ids output: %s\n", prefab_path );
	std::size_t prefab_count = 0;
	auto prefabs_written = write_prefab_ids( prefab_path, prefab_count );
	if ( prefabs_written )
		std::printf( "[morphine-dumper] prefab ids written to %s (%zu entries)\n", prefab_path, prefab_count );
	else
		std::printf( "[morphine-dumper] prefab ids failed to write\n" );

	char live_prefab_path[ MAX_PATH ]{ };
	std::strncpy( live_prefab_path, output_path, MAX_PATH - 1 );
	if ( auto slash = std::strrchr( live_prefab_path, '\\' ) )
		std::snprintf( slash + 1, MAX_PATH - ( slash - live_prefab_path + 1 ), "live_prefab_ids.h" );
	else
		std::snprintf( live_prefab_path, MAX_PATH, "live_prefab_ids.h" );

	std::vector<prefab_entry_t> live_prefab_entries;
	std::size_t live_prefab_count = 0;
	if ( collect_live_prefab_ids(
		live_prefab_entries,
		m_bn.base_networkable_rva,
		m_bn.static_fields,
		m_bn.wrapper_class_ptr,
		...
		m_bn.decrypt_0,
		m_bn.decrypt_1 ) &&
		write_prefab_entries( live_prefab_path, live_prefab_entries, live_prefab_count ) )
	{
		std::printf( "[morphine-dumper] live prefab ids written to %s (%zu entries, paths unavailable)\n",
			live_prefab_path,
			live_prefab_count );
	}
	...
	auto verification_passed =
		m_bn_verified &&
		m_camera_verified &&
		g_cl_active_item_verified &&
		g_player_inventory_verified &&
		g_player_eyes_verified &&
		decryptions_distinct &&
		m_fov_constants.valid &&
		prefabs_written;

	char verification_path[ MAX_PATH ]{ };
	std::strncpy( verification_path, output_path, MAX_PATH - 1 );
	if ( auto slash = std::strrchr( verification_path, '\\' ) )
		std::snprintf(
			slash + 1,
			MAX_PATH - ( slash - verification_path + 1 ),
			"verification.txt" );
	else
		std::snprintf( verification_path, MAX_PATH, "verification.txt" );

	if ( auto verification_file = std::fopen( verification_path, "w" ) )
	{
		std::fprintf( verification_file, "build=%s\n", m_build_id.c_str( ) );
		std::fprintf( verification_file, "BaseNetworkable=%s\n", m_bn_verified ? "PASS" : "FAIL" );
		std::fprintf( verification_file, "Camera=%s\n", m_camera_verified ? "PASS" : "FAIL" );
		std::fprintf( verification_file, "clActiveItem=%s\n", g_cl_active_item_verified ? "PASS" : "FAIL" );
		std::fprintf( verification_file, "PlayerInventory=%s\n", g_player_inventory_verified ? "PASS" : "FAIL" );
		std::fprintf( verification_file, "PlayerEyes=%s\n", g_player_eyes_verified ? "PASS" : "FAIL" );
		std::fprintf( verification_file, "FOV=%s\n", m_fov_constants.valid ? "PASS" : "FAIL" );
		std::fprintf( verification_file, "Prefabs=%s\n", prefabs_written ? "PASS" : "FAIL" );
		std::fprintf( verification_file, "Overall=%s\n", verification_passed ? "PASS" : "FAIL" );
		std::fclose( verification_file );
	}

	std::printf(
		"[morphine-dumper] verification summary: bn=%s camera=%s clActiveItem=%s inventory=%s eyes=%s fov=%s prefabs=%s overall=%s\n",
		m_bn_verified ? "PASS" : "FAIL",
		m_camera_verified ? "PASS" : "FAIL",
		g_cl_active_item_verified ? "PASS" : "FAIL",
		g_player_inventory_verified ? "PASS" : "FAIL",
