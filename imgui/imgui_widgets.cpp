...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float text_base_offset = ImMax(0.0f, (square_sz - label_size.y) * 0.5f); 

    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), ImMax(square_sz, label_size.y) + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (*v)
    {
        ...
        ImU32 check_col = IM_COL32(188, 88, 27, 255);
    ...
    if (mixed_value)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
    }


    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + text_base_offset);
    if (g.LogEnabled)
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_width = GetFrameHeight() * 1.8f; 
    const float default_height = GetFrameHeight() -4.f;
    const ImVec2 size(size_arg.x == 0.0f ? default_width : size_arg.x, size_arg.y == 0.0f ? default_height : size_arg.y);

    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    ItemSize(bb, (size.y >= default_height) ? g.Style.FramePadding.y : 0.0f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    float grid_step_x = size.x / 5.99f; 
    float grid_step_y = size.y / 1.99f; 
    float rounding = ImMin(g.Style.FrameRounding, ImMin(grid_step_x, grid_step_y) * 0.5f); 
    ImRect bb_inner = bb;
    float off = 0.0f;
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step_x, bb_inner.Min.y), bb_inner.Max, GetColorU32(col_rgb), grid_step_x, ImVec2(-grid_step_x + off, off), rounding, ImDrawFlags_RoundCornersRight);
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step_x, ImVec2(off, off), rounding);
        else
